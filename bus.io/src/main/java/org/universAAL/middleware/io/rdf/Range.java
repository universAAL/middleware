...
public class Range extends Input {
	...
	static {
		addResourceClass(MY_URI, Range.class);
	}
	...
	public Range(Group parent, Label label, PropertyPath ref,
			Restriction valueRestriction, Object initialValue) {
	...
	private Comparable getBound(Restriction r, boolean upper) {
		if (r == null)
			return null;

		Comparable res = null;
		if (r instanceof OrderingRestriction) {
			res = upper ? ((OrderingRestriction) r).getUpperbound()
					: ((OrderingRestriction) r).getLowerbound();
			if (res != null)
				return res;
		}

		Class claz = ManagedIndividual.getRegisteredClass(r.getPropTypeURI());
		return (claz != null && ComparableIndividual.class
				.isAssignableFrom(claz)) ? (upper ? ComparableIndividual
				.getClassMaximum(claz) : ComparableIndividual
				.getClassMinimum(claz)) : null;
	}
