...
public class OutputStrategy extends BusStrategy {
	...
	case MessageType.P2P_REQUEST:
		...
		SharedResources.assessContentSerialization(res);
		...
		if (handler != null
			...
			if (userInput != null)
			    ie.setProperty(InputEvent.PROP_DIALOG_DATA,
				    userInput);
			SharedResources.assessContentSerialization(ie);
	...
	if (dialogManager == null && theCoordinator == null) {
	    ...
	    SharedResources.assessContentSerialization(r);
	...
	if (sodapop.getID().equals(peerID)) {
		...
		LogUtils.logInfo(SharedResources.moduleContext,
			OutputStrategy.class, "notifyHandler_apChanged",
	...
	if (sodapop.getID().equals(peerID)) {
	    ...
	    if (o instanceof OutputSubscriber) {
		LogUtils.logInfo(SharedResources.moduleContext,
			OutputStrategy.class, "notifyHandler_handle",
			new Object[] { "Notified handler ", handlerID, ":\n",
				content }, null);
