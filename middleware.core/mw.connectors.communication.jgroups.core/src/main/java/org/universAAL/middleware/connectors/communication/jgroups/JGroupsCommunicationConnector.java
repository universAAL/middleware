...
public class JGroupsCommunicationConnector
		implements CommunicationConnector, Receiver, RequestHandler, MembershipListener {
	...
	public JGroupsCommunicationConnector(ModuleContext context) throws Exception {
		...
		security = Boolean.parseBoolean(System.getProperty("universaal.security.enabled", "false"));
	...
	private void initializeSecurity() throws Exception {
		...
		LogUtils.logDebug(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
		...
		LogUtils.logDebug(context, JGroupsCommunicationConnector.class, METHOD,
				new Object[] { "Security key file : ", fileName, " bouncycastle.key" }, null);
		...
		if (!exists) {
			// disable security
			LogUtils.logDebug(context, JGroupsCommunicationConnector.class, METHOD,
					"Security disabled, key file not found");
			...
			try {
				...
				LogUtils.logError(context, JGroupsCommunicationConnector.class, METHOD,
						new Object[] { "Error while initializing the CryptoUtil: ", ex.toString() }, ex);
				throw new Exception("Security disabled. Key file not found.");
			}

			LogUtils.logDebug(context, JGroupsCommunicationConnector.class, METHOD, "Connector Up");
		}
	}
	...
	public void configureConnector(List<ChannelDescriptor> channels, String peerName)
		...
		LogUtils.logDebug(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
				new Object[] { "Configuring the JChannel and the ReceiverAdapter..." }, null);
		communicationModule = (CommunicationModule) context.getContainer().fetchSharedObject(context,
		...
		for (final ChannelDescriptor element : channels) {
			try {
				...
				if (channelMap.containsKey(element.getChannelName())) {
					LogUtils.logWarn(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
							new Object[] { "The channel: " + element.getChannelName() + " is already configured" },
							null);
				} else {
					try {
						ch = configureJChannel(element);
					} catch (Exception e) {
						LogUtils.logError(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
								new Object[] { "Error configuringing the JChannel" + e }, null);
						throw new CommunicationConnectorException(CommunicationConnectorErrorCode.NEW_CHANNEL_ERROR,
				...
				LogUtils.logError(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
						new Object[] { "Error configuringing the JChannel and the ReceiverAdapter: " + e }, null);
				throw new CommunicationConnectorException(CommunicationConnectorErrorCode.NEW_CHANNEL_ERROR,
						"Error configuringing the JChannel and the ReceiverAdapter: " + e);
			}
		}
		LogUtils.logDebug(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
				new Object[] { "JChannel and ReceiverAdapter configured." }, null);
	 ...
	 * @throws Exception
	 */
	private JChannel configureJChannel(ChannelDescriptor element) throws Exception {
		...
		if (enableRemoteChannelConfigurarion && urlConfig == null && element.getChannelDescriptorFileURL() != null) {
		...
		if (urlConfig != null) {
			try {
				LogUtils.logDebug(context, JGroupsCommunicationConnector.class, METHOD,
						new Object[] { "Loading JGroups communication channel from ", urlConfig }, null);
				ch = createSharedChannel(urlConfig);
			} catch (Exception e) {
				LogUtils.logError(context, JGroupsCommunicationConnector.class, METHOD,
						new Object[] { "Failed to load remote configuration for ", element.getChannelName(),
								" from URL -> ", urlConfig, " due to internal exception ",
								ExceptionUtils.stackTraceAsString(e), "\n",
								"Trying to initializee the channels locally" },
						e);
			}
		}
		if (enableRemoteChannelConfigurarion && ch == null && element.getChannelValue() == null) {
			LogUtils.logDebug(context, JGroupsCommunicationConnector.class, METHOD,
					new Object[] { "No local configuration for ", element.getChannelName(),
		...
		if (enableRemoteChannelConfigurarion && ch == null && element.getChannelValue() != null) {
			...
			InputStream channelValue = new ByteArrayInputStream(element.getChannelValue().getBytes());
			try {
				LogUtils.logDebug(context, JGroupsCommunicationConnector.class, METHOD,
				...
				LogUtils.logError(context, JGroupsCommunicationConnector.class, METHOD,
						new Object[] { "Failed to load local configuration for ", element.getChannelName(),
								" due to internal exception ", ExceptionUtils.stackTraceAsString(e), "\n",
								"Falling back to JGroup default cluster configuration" },
						e);
			}
		...
		if (ch != null) {
			...
			throw new CommunicationConnectorException(CommunicationConnectorErrorCode.CHANNEL_INIT_ERROR,
	...
	public void dispose(List<ChannelDescriptor> channels) {
		LogUtils.logDebug(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
				new Object[] { "Reset the JGroupCommunicationConnector..." }, null);
		try {
			if (channelMap != null) {
			...
			LogUtils.logError(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
					new Object[] { "Error while resetting the Communication connector: " + e.toString() }, null);
		} finally {
	...
	private JChannel createSharedChannel(URL channelURL) throws Exception {
		try {
			ProtocolStackConfigurator config = ConfiguratorFactory.getStackConfigurator(channelURL);
			...
			LogUtils.logError(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
					new Object[] { "Unable to initialize the JGroup channel with URL: ", channelURL }, e);
			throw new CommunicationConnectorException(CommunicationConnectorErrorCode.CHANNEL_INIT_ERROR,
					"Unable to initialize the JGroup channel with URL: " + channelURL, e);
		}

	}

	private JChannel createSharedChannel(InputStream channelValue) throws Exception {
		try {
			ProtocolStackConfigurator config = ConfiguratorFactory.getStackConfigurator(channelValue);
			List<ProtocolConfiguration> protocols = config.getProtocolStack();
			// ProtocolConfiguration transport = protocols.get(0);
			// transport.getProperties().put(Global.SINGLETON_NAME,
			// transport.getProtocolName());
			...
			LogUtils.logError(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
					new Object[] { "Unable to initialize the JGroup channel with URL: " + channelValue.toString()
							+ " -> " + e.toString() },
					null);
			throw new CommunicationConnectorException(CommunicationConnectorErrorCode.CHANNEL_INIT_ERROR,
					"Unable to initialize the JGroup channel with URL: " + channelValue.toString() + " -> "
							+ e.toString());
		}
	}
	...
	private List<JChannel> selectJChannels(List destinationChannelNames) {
		...
		for (int i = 0; i < destinationChannelNames.size(); i++) {
			String destinaltionChannelName = (String) destinationChannelNames.get(i);
	...
	public synchronized void unicast(ChannelMessage message, String receiver) throws CommunicationConnectorException {
		final String METHOD = "unicast";
		if (message.getChannelNames() == null || message.getChannelNames().isEmpty()) {
			logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.NO_CHANNEL_SPECIFIED,
					"No channel name specified");
			return;
		}
		if (message.getChannelNames().size() > 1) {
			logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.MULTIPLE_RECEIVERS,
		...
		if (ch == null) {
			logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.CHANNEL_NOT_FOUND,
					"The channel name (channel is null) was not found. It is either not configured or it has been deleted");
			return;
		}
		View view = ch.getView();
		if (view == null) {
			logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.NOT_CONNECTED_TO_CHANNEL,
					"Unable to get the View on the channel " + ch.getName() + " We may not be connected to it");
			return;
		}
		Address dst = null;
		final Message msg;
		...
		if (dst == null) {
			logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.RECEIVER_NOT_EXISTS, "Trying to send message to "
					+ receiver + " but it is not a memeber of " + ch.getName() + "/" + ch.getClusterName());
			return;
		}
		if (security) {
			try {
				msg = new Message(dst, null, CryptUtil.encrypt(message.toString()));
			} catch (Throwable t) {
				logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.SEND_MESSAGE_ERROR,
						"Failed to encrypt the message due to internal exception", t);
				return;
			}
		} else {
		...
		try {
			...
			logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.SEND_MESSAGE_ERROR,
					"Error sending unicast message " + message + " due to internal exception", t);
			return;
		}
	}

	private void logAndThrowComExec(String method, CommunicationConnectorErrorCode code, String msg, Throwable t) {
		LogUtils.logError(context, JGroupsCommunicationConnector.class, method, new Object[] { msg }, t);
		throw new CommunicationConnectorException(CommunicationConnectorErrorCode.NO_CHANNEL_SPECIFIED, msg, t);

	}

	private void logAndThrowComExec(String method, CommunicationConnectorErrorCode code, String msg) {
		LogUtils.logError(context, JGroupsCommunicationConnector.class, method, msg);
		throw new CommunicationConnectorException(CommunicationConnectorErrorCode.NO_CHANNEL_SPECIFIED, msg);
	}

	public synchronized void multicast(ChannelMessage message) throws CommunicationConnectorException {
		...
		for (int i = 0; i < selectedChannel.size(); i++) {
			...
			try {
				if (security) {
					msg = new Message(null, null, CryptUtil.encrypt(message.toString()));
				} else {
	...
	public synchronized void multicast(ChannelMessage message, List<PeerCard> receivers)
			throws CommunicationConnectorException {
		final String METHOD = "multicast";
		...
		if (selectedChannel == null || selectedChannel.isEmpty()) {
			logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.CHANNEL_NOT_FOUND,
					"No destination channel found among the list :" + Arrays.toString(channels.toArray())
		...
		for (int i = 0; i < selectedChannel.size(); i++) {
			...
			try {
				...
				logAndThrowComExec(METHOD, CommunicationConnectorErrorCode.SEND_MESSAGE_ERROR,
						"Unable to broadcast the message:" + message.toString(), e);
				return;
			}
		}
	...
	public void loadConfigurations(Dictionary configurations) {
		...
		try {
			...
			this.enableRemoteChannelConfigurarion = Boolean
					.valueOf((String) configurations.get(Consts.ENABLE_REMOTE_CHANNEL_CONFIG));
			this.enableRemoteChannelURL = (String) configurations.get(Consts.ENABLE_REMOTE_CHANNEL_URL_CONFIG);
		} catch (Throwable t) {
			LogUtils.logError(context, JGroupsCommunicationConnector.class, METHOD,
					new Object[] { "Error during JGroups properties update" }, t);
		}
		LogUtils.logDebug(context, JGroupsCommunicationConnector.class, "JGroupsCommunicationConnector",
	...
	public void receive(Message msg) {
		...
		try {
			...
			LogUtils.logDebug(context, JGroupsCommunicationConnector.class, METHOD,
					new Object[] {
							"Failed to unmarhall message due to exception " + ExceptionUtils.stackTraceAsString(ex) },
					ex);
		}
	}
	...
	public String toString() {
		return "Name: " + this.name + " Version: " + this.version + " Description: " + this.description + " Provider: "
				+ this.provider;
	}

	public Object handle(Message msg) throws Exception {
		ChannelMessage channelMessage = ChannelMessage.unmarshall(new String(msg.getBuffer()));
		communicationModule.messageReceived(channelMessage);
		return null;
	}
	...
	public List<String> getGroupMembers(String groupName) {
		...
		for (String channelName : channelMap.keySet()) {
			if (channelName.contains(groupName) && channelMap.get(channelName).getView() != null) {
				List<Address> addresses = channelMap.get(channelName).getView().getMembers();
