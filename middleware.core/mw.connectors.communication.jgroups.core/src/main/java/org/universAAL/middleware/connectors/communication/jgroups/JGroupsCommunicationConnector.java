        ...
        if (message.getChannelNames() == null
            ...
            if (ch == null) {
                ...
                for (Address address : list) {                  
                    /*
                     * //FIX The android peer is not shown as member of the channel thus joining fails
                     * //GOODHACK Add a list of peers that are always on
                     * //BADHACK Remove the control 
                     */                  
                    final boolean check = Boolean.valueOf( System.getProperty("org.uAAL.mw.comunication.peer_exists_check","true") );
                    if (!check || receiver.equals(ch.getName(address))) { 
                        if (security) {
                            try {
