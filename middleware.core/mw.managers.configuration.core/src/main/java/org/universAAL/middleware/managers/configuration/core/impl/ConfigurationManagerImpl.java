...
public class ConfigurationManagerImpl implements ConfigurationManager,
	ConfigurationManagerConnector, ConfigurationEditor,
	DynamicDescribedEntityListener {

    static final String PROP_PARAM = AALConfigurationOntology.NAMESPACE
	    + "messageParameter";
    static final String PROP_LOCALE = AALConfigurationOntology.NAMESPACE
	    + "preferredLocale";
	...
	for (DescribedEntity de : confPattern) {
		...
		moduleRegistry.put(ScopeFactory.getScopeURN(de.getScope()),
			listener);
	    } else {
	...
	/*
	 * filter out remotes (locals for other nodes) so they are not stored
	 * locally. IE: after filtering, the remainder must be
	 * not(instanceScope)|localInstance
	 */
	Union u = new Union();
	...
	if (e instanceof ConfigurationFile) {
	    ...
	    if (url != null) {
		/*
		 * check it is local, it exists ; if not make it local by
		 * caching it
		 */
		...
		if (value instanceof File) {
		    ...
		    try {
			((ConfigurationFile) e).setLocalURL(f.toURI().toURL()
				.toString());
			e.incrementVersion();
			manager.addEntity(e);
		...
		if (!ConfigurationFile.writeContentToFile(content, f)) {
		    LogUtils.logError(context, getClass(), "updateLocalValue",
			    "unable to write content in file: " + f);
		    return false;
		}
	    } else {
		...
		try {
		    ...
		    LogUtils.logWarn(
			    ...
			    getClass(),
			    "updateLocalValue",
			    "unable lo load content from URL: "
				    + cf.getLocalURL()
			...
			LogUtils.logError(
				...
				getClass(),
				"updateLocalValue",
				"unable lo load content from defaultURL: "
					+ cf.getDefaultURL());
			return false;
		    }
		}
	...
	if (value != null)
	    return moduleRegistry.configurationChanged(
		    ScopeFactory.getScope(e.getURI()), value);
	return false;
    }
	...
	if (ne instanceof ConfigurationFile) {
	    ...
	    if (value instanceof File) {
		try {
		    ((ConfigurationFile) ne).loadContentFormURL(((File) value)
			    .toURI().toURL());
		...
		try {
		    ...
		    LogUtils.logError(context, getClass(), "updatedValue",
			    new String[] { "unable to load new url" }, e);
		    return;
		}
	...
	for (Entity e : stored) {
	    try {
		Entity ne = EntityFactory.updateEntity(e,
			entitiesSources.get(e.getURI()), locale);
		manager.addEntity(ne);
		// use ConfigurationEditorPool
	    } catch (Exception e1) {
		/*
		 * described entity is not local. create a remoteEditor and let
		 * it manage it (will be done by editorsPool.get(Entity)).
		 */
	    }
	    cmanager.addConfigurableEntity(editorsPool.get(e));
	}
	/*
	 * add filter to the request, remove : - global configs (they are stored
	 * and maintained on all nodes. require: - local (to recipient) config
	 * only that match the filters.
	 */
	...
	String localPeerID = shared.getAalSpaceManager().getMyPeerCard()
		.getPeerID();
	...
	ConfigurationMessage cm = new ConfigurationMessage(
		ConfigurationMessageType.QUERY, shared.getAalSpaceManager()
			.getMyPeerCard(), shared.getMessageContentSerializer()
			.serialize(root));
	// send
	...
	String serialized = shared.getMessageContentSerializer()
		.serialize(root);
	ConfigurationMessage cm = new ConfigurationMessage(
		ConfigurationMessageType.PROPAGATE, shared.getAalSpaceManager()
			.getMyPeerCard(), serialized);
	...
	Object r = shared.getMessageContentSerializer().deserialize(
		message.getPayload());
	if (r instanceof Resource && ((Resource) r).hasProperty(PROP_PARAM)) {
	    ...
	    if (cand instanceof List && !((List) cand).isEmpty()
	...
	if (r instanceof Resource && ((Resource) r).hasProperty(PROP_PARAM)) {
		...
		if (response != null && !response.isEmpty()) {
		    ...
		    List<Entity> filtered = EntityManager.filter(response,
			    asList(localOnlyExpression()));
		    List<Entity> processed = new ArrayList<Entity>();
		    for (Entity e : filtered) {
			try {
			    e = EntityFactory.updateEntity(e, entitiesSources
				    .get(e.getURI()), (Locale) ((Resource) r)
		    ...
		    ConfigurationMessage cm = message.createResoponse(shared
			    .getMessageContentSerializer().serialize(res));
		    // send
		    shared.getControlBroker().sendConfigurationMessage(cm);
		}
	    }
