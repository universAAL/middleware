...
public class ConfigurationManagerImpl implements ConfigurationManager, ConfigurationManagerConnector,
	...
	/** {@ inheritDoc} */
	public void register(DescribedEntity[] confPattern, ConfigurableModule listener) {
	...
	boolean updateLocalValue(Entity e) {
		...
		if (e instanceof ConfigurationFile) {
			...
			if (url != null) {
				 ...
				 * caching it
				 */
				if ("file".equalsIgnoreCase(url.getProtocol()) || url.getProtocol() == null
			...
			if (content != null) {
				...
				try {
					...
					LogUtils.logWarn(context, getClass(), "updateLocalValue",
							"unable lo load content from URL: " + cf.getLocalURL() + "\n Trying to load Default.");
					try {
						...
						LogUtils.logError(context, getClass(), "updateLocalValue",
								"unable lo load content from defaultURL: " + cf.getDefaultURL());
						return false;
					}
				}
		...
		if (ne instanceof ConfigurationFile) {
			...
			if (value instanceof File) {
				try {
					...
					LogUtils.logError(context, getClass(), "udpdatedValue", "wrong URL.");
				} catch (IOException e) {
					LogUtils.logError(context, getClass(), "udpdatedValue", "can't access file: " + value);
				}
				...
				try {
					...
					LogUtils.logError(context, getClass(), "updatedValue", new String[] { "unable to load new url" },
							e);
					return;
				}
	...
	/** {@ inheritDoc} */
	public List<ConfigurableEntityEditor> getMatchingConfigurationEditors(List<EntityPattern> configPattern,
			Locale locale) {
	...
	/** {@ inheritDoc} */
	public void registerConfigurableEntityManager(ConfigurableEntityManager cmanager, List<EntityPattern> configPattern,
			Locale locale) {
	...
	public void processPropagation(ConfigurationMessage message) {
		...
		if (r instanceof Resource && ((Resource) r).hasProperty(PROP_PARAM)) {
			...
			if (cand instanceof List && !((List) cand).isEmpty() && ((List) cand).get(0) instanceof Entity) {
			...
			if (cand instanceof List) {
				...
				if (response != null && !response.isEmpty()) {
					...
					for (Entity e : filtered) {
						try {
							e = EntityFactory.updateEntity(e, entitiesSources.get(e.getURI()),
									(Locale) ((Resource) r).getProperty(PROP_LOCALE));
							processed.add(e);
						} catch (Exception e1) {
					...
					ConfigurationMessage cm = message
							.createResoponse(shared.getMessageContentSerializer().serialize(res));
					// send
					shared.getControlBroker().sendConfigurationMessage(cm);
				}
			}
