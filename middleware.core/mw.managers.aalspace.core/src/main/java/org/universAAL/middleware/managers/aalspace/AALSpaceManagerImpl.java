...
public class AALSpaceManagerImpl implements AALSpaceEventHandler,
	...
	public synchronized void initAALSpace(IAALSpace aalSpaceDefaultConfiguration) {
		...
		try {
			...
			if (spaceCards != null && spaceCards.size() > 0) {
				...
				/**
				 * No AALSpaces have been found: 1) if I'm the Coordinator and
				 * the aalSpaceDefaultConfiguration does not contain Coordinator
				 * ID -> create new AALSpace 2) if the
				 * aalSpaceDefaultConfiguration contains Coordinator ID -> join
				 * existing AALSpace 3) otherwise I wait joining
				 */
				if (getMyPeerCard().getRole().equals(PeerRole.COORDINATOR)
						&& !isSetCoordinatorID(aalSpaceDefaultConfiguration)) {
					...
					LogUtils.logInfo(context, AALSpaceManagerImpl.class,
							"initAALSpace", new Object[] {
									"-->Waiting to join an AALSpace as :",
	...
	private Dictionary<String, String> getAALSpaceProperties(IAALSpace space) {
		...
		try {
			...
			// If the aalSpaceDefaultConfiguration contains the coordinator ID,
			// I store it
	 ...
	 * 
	 * @param aalSpaceDefaultConfiguration
	 * @return if the aalSpaceDefaultConfiguration and it is different from my
	 *         peerID
	 */
	private boolean isSetCoordinatorID(IAALSpace aalSpaceDefaultConfiguration) {
		if (aalSpaceDefaultConfiguration.getSpaceDescriptor()
				.getSpaceCoordinator() != null
				&& !aalSpaceDefaultConfiguration.getSpaceDescriptor()
						.getSpaceCoordinator().isEmpty()
				&& !aalSpaceDefaultConfiguration.getSpaceDescriptor()
						.getSpaceCoordinator().equals(myPeerCard.getPeerID())) {
			return true;
		} else
			return false;
	}
