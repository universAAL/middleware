...
public class AALSpaceManagerImpl
    ...
    public AALSpaceManagerImpl(ModuleContext context, String altDir) {
	...
	try {
	    TIMEOUT = Long.parseLong(
		    System.getProperty(AALSpaceManager.COMUNICATION_TIMEOUT_KEY,
			    AALSpaceManager.COMUNICATION_TIMEOUT_VALUE));
	} catch (Exception ex) {
	    LogUtils.logError(context, AALSpaceManagerImpl.class,
		    "AALSpaceManagerImpl",
		    new Object[] {
			    "intalization timeout, falling back to default value: "
				    + AALSpaceManager.COMUNICATION_TIMEOUT_VALUE },
		    null);
	    TIMEOUT = Long
	...
	String peerId = System.getProperty(
		org.universAAL.middleware.managers.aalspace.util.Consts.PEER_ID);
	if (peerId == null) {
	...
	if (!initialized) {
	    ...
	    Object[] cBrokers = context.getContainer()
		    .fetchSharedObject(context,
			    new Object[] {
				    ControlBroker.class.getName().toString() },
			    this);
	    if (cBrokers != null) {
		LogUtils.logDebug(context, AALSpaceManagerImpl.class, "init",
		...
		LogUtils.logWarn(context, AALSpaceManagerImpl.class, "init",
			new Object[] {
				"AALSpace default configurations are null" },
			null);
		initialized = true;
	    } else {
		LogUtils.logDebug(context, AALSpaceManagerImpl.class, "init",
			new Object[] {
				"Parse the AALSpace default configurations" },
			null);
     ...
     */
    public synchronized void initAALSpace(
	    IAALSpace aalSpaceDefaultConfiguration) {
	...
	if (aalSpaceDefaultConfiguration == null) {
	    ...
	    LogUtils.logDebug(context, AALSpaceManagerImpl.class,
		    ...
		    new Object[] {
			    ...
			    aalSpaceConfigurationPath, " giving up" },
		    null);
	    return;
	}
	try {
	    ...
	    List<AALSpaceCard> spaceCards = controlBroker.discoverAALSpace(
		...
		if (getMyPeerCard().getRole().equals(PeerRole.COORDINATOR)) {
		    ...
		    LogUtils.logInfo(context, AALSpaceManagerImpl.class,
			    "initAALSpace",
			    new Object[] {
				    "No default AALSpace found...creating it " },
			    null);
		    ...
		    ChannelDescriptor peeringChannel = getChannel(
			    aalSpaceDefaultConfiguration.getPeeringChannel()
				    .getChannelDescriptor());
			...
			LogUtils.logWarn(context, AALSpaceManagerImpl.class,
				"initAALSpace",
				new Object[] {
					"Control Broker is not initialize" },
				null);
		    }
	...
	if (currentAALSpace != null) {
	    LogUtils.logWarn(context, AALSpaceManagerImpl.class, "join",
		    new Object[] {
			    "Cannot join to multiple AALSpace. First leave the current AALSpace " },
		    null);
	...
	if (init()) {
		...
		if (defaultPeeringChannel != null
			&& peeringChannelSerialized.equals(defaultPeeringChannel
		    ...
		    peeringChannel.setChannelDescriptorFileURL(
		...
		try {
			...
			LogUtils.logInfo(context, AALSpaceManagerImpl.class,
				"join",
				new Object[] {
					"--->Peering channel configured!" },
				null);
		    } else {
			LogUtils.logWarn(context, AALSpaceManagerImpl.class,
				"join",
				new Object[] {
					"Peering channel is null not able to join the AALSpace" },
				null);
		    }
		    LogUtils.logInfo(context, AALSpaceManagerImpl.class, "join",
	...
	if (init()) {
		...
		try {
		    ...
		    LogUtils.logError(context, AALSpaceManagerImpl.class,
			    "aalSpaceJoined",
			    new Object[] {
				    "Error during notify: " + e.toString() },
			    null);
		}
	    }
	    ...
	    checkPeerThread = new CheckPeerThread(context);
	    checkerFuture = scheduler.scheduleAtFixedRate(checkPeerThread, 0, 1,
		    TimeUnit.SECONDS);
	    ...
	    LogUtils.logWarn(context, AALSpaceManagerImpl.class,
		    "aalSpaceJoined",
		    new Object[] {
			    "AALSpace Manager is not initialized aborting." },
		    null);
	    pendingAALSpace = false;
	}
	...
	if (space != null) {
	    try {
		filters.put(Consts.AALSPaceID,
			space.getSpaceDescriptor().getSpaceId());
		LogUtils.logDebug(context, AALSpaceManagerImpl.class,
			"buildAALSpaceFilter",
		...
		LogUtils.logError(context, AALSpaceManagerImpl.class,
			"buildAALSpaceFilter",
			new Object[] {
				"Error while building AALSpace filter...returning empty filter"
					+ e.toString() },
			null);
		return filters;
	    } catch (Exception e) {
	...
	try {
	    ...
	    properties.put(Consts.AALSPaceName,
		    space.getSpaceDescriptor().getSpaceName());
	    properties.put(Consts.AALSPaceID,
		    space.getSpaceDescriptor().getSpaceId());
	    properties.put(Consts.AALSPaceDescription,
		    space.getSpaceDescriptor().getSpaceDescription());
	    properties.put(Consts.AALSpaceCoordinator,
		    getMyPeerCard().getPeerID());
	    ...
	    properties.put(Consts.AALSpacePeeringChannelName,
		    space.getPeeringChannel().getChannelDescriptor()
			    .getChannelName());
	    properties.put(Consts.AALSPaceProfile,
		    space.getSpaceDescriptor().getProfile());
	...
	if (spaceConfigDirectory.canRead() == false) {
	    LogUtils.logWarn(context, AALSpaceManagerImpl.class,
		    "getFileList", new Object[] { "File: "
			    + aalSpaceConfigurationPath + " cannot be read." },
		    null);
	    return null;
	}
	...
	if (spaces == null || spaces.length == 0) {
	    LogUtils.logError(context, AALSpaceManagerImpl.class, METHOD,
		    new Object[] { "Both directory ", aalSpaceConfigurationPath,
			    " and ", altConfigDir,
	...
	// }
	// alspace space = (Aalspace) unmarshaller
	// .unmarshal(defaultSpaceConfiguration);
	// PATCH: HARDCODED CONFIGURATION String value = "<![CDATA[<config
	// xmlns=\"urn:org:jgroups\"
	// xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\"
	// xsi:schemaLocation=\"urn:org:jgroups
	// http://www.jgroups.org/schema/JGroups-3.0.xsd\"> <UDP
	...
	// ip_ttl=\"${jgroups.udp.ip_ttl:8}\" enable_bundling=\"true\"
	// enable_diagnostics=\"false\" thread_naming_pattern=\"cl\"
	...
	// oob_thread_pool.rejection_policy=\"Run\"/> <PING timeout=\"2000\"
	// num_initial_members=\"3\"/> <MERGE2 max_interval=\"30000\"
	// min_interval=\"10000\"/> <FD_SOCK/> <FD_ALL/> <VERIFY_SUSPECT
	// timeout=\"1500\" /> <BARRIER /> <pbcast.NAKACK
	// exponential_backoff=\"300\" xmit_stagger_timeout=\"200\"
	// use_mcast_xmit=\"false\" discard_delivered_msgs=\"true\"/> <UNICAST
	// /> <pbcast.STABLE stability_delay=\"1000\"
	// desired_avg_gossip=\"50000\" max_bytes=\"4M\"/> <pbcast.GMS
	// print_local_addr=\"true\" join_timeout=\"3000\"
	// view_bundling=\"true\"/> <UFC max_credits=\"2M\"
	// min_threshold=\"0.4\"/> <MFC max_credits=\"2M\"
	// min_threshold=\"0.4\"/> <FRAG2 frag_size=\"60K\" />
	// <pbcast.STATE_TRANSFER /> <pbcast.FLUSH /> </config>]]>";
	...
	for (IChannelDescriptor channelDescriptor : space
	    ...
	    channelDescriptor.setChannelName(
		    channelDescriptor.getChannelName() + aalSpaceID);
	}
	return space;
    }
    ...
    // METHOD,
    // new Object[] {
    // "Error during AALSpace schema initialization for validatin XML, it will
    // not be used"
    // },
    // ex);
    // }
	...
	LogUtils.logDebug(context, AALSpaceManagerImpl.class,
		...
		null);
	String role = (String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.PEER_ROLE);
	String roleOverride = System.getProperty(
		org.universAAL.middleware.managers.aalspace.util.Consts.PEER_ROLE);
	...
	LogUtils.logDebug(context, AALSpaceManagerImpl.class,
		...
		null);
	aalSpaceConfigurationPath = (String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.AAL_SPACE_CONFIGURATION_PATH);
	try {
	    if (aalSpaceConfigurationPath == null) {
		LogUtils.logWarn(context, AALSpaceManagerImpl.class,
			"loadConfigurations",
	...
	spaceExtension = (String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.SPACE_EXTENSION);
	aalSpaceValidation = Boolean.parseBoolean((String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.AAL_SPACE_VALIDATION));
	aalSpaceSchemaURL = (String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.AAL_SPACE_SCHEMA_URL);
	aalSpaceLifeTime = Integer.parseInt((String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.AAL_SPACE_LIFETIME));

	aalSpaceSchemaName = (String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.AAL_SPACE_SCHEMA_NAME);
	waitBeforeClosingChannels = Long.parseLong((String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.WAIT_BEFEORE_CLOSING_CHANNEL));
	waitAfterJoinRequest = Long.parseLong((String) configurations.get(
		org.universAAL.middleware.managers.aalspace.util.Consts.WAIT_BEFEORE_CLOSING_CHANNEL));
	...
	if (init()) {
		...
		if (peers.containsKey(peer.getPeerID())) {
		    LogUtils.logError(context, AALSpaceManagerImpl.class,
			    "joinRequest",
			    new Object[] {
				    "A peer with this ID is already available in the space. Ignoring request" },
			    null);
		    return;
		}
		...
		if (!managedAALspaces.containsKey(spaceCard.getSpaceID())) {
		    LogUtils.logWarn(context, AALSpaceManagerImpl.class,
			    "joinRequest",
			    new Object[] {
				    "Received a join request to an AALSpace not managed: my AALSpace: "
	...
	if (spaceCards != null) {
		...
		LogUtils.logTrace(context, AALSpaceManagerImpl.class,
			"newAALSpacesFound",
			new Object[] {
				"--->The list of AAL Spaces has been updated:"
	...
	if (peer != null && !peers.containsKey(peer.getPeerID())) {
	    LogUtils.logInfo(context, AALSpaceManagerImpl.class, "peerFound",
		    new Object[] {
			    "--->The Peer: " + peer.getPeerID().toString()
				    + " joins the AALSpace: " },
		    null);
	...
	if (init()) {
	    if (spaceDescriptor != null && managedAALspaces
		    .containsKey(spaceDescriptor.getSpaceCard().getSpaceID())) {
		LogUtils.logInfo(context, AALSpaceManagerImpl.class,
			"leaveAALSpace",
		...
		LogUtils.logInfo(context, AALSpaceManagerImpl.class,
			"leaveAALSpace",
			new Object[] {
				"--->Leaving the AALSpace: " + spaceDescriptor
					.getSpaceCard().getSpaceName() },
			null);
		PeerCard spaceCoordinator = new PeerCard(
			spaceDescriptor.getSpaceCard().getCoordinatorID(),
			PeerRole.COORDINATOR);
		controlBroker.leaveAALSpace(spaceCoordinator,
			spaceDescriptor.getSpaceCard());
		LogUtils.logDebug(context, AALSpaceManagerImpl.class,
			"leaveAALSpace", new Object[] { "Leave message sent!" },
			null);
	...
	if (init()) {
		...
		try {
		    ...
		    LogUtils.logError(context, AALSpaceManagerImpl.class,
			    "dispose",
			    new Object[] {
				    "Error during dispose: " + e.toString() },
			    null);
		}
	    } else {
		if (currentAALSpace != null)
	...
	try {
	    ...
	    LogUtils.logError(context, AALSpaceManagerImpl.class,
		    "closeManagedSpace",
		    new Object[] { "Error during wait: " + e.toString() },
		    null);
	}
	ChannelDescriptor peeringChannel = new ChannelDescriptor(
		spaceDescriptor.getSpaceCard().getPeeringChannelName(), "",
	...
	// coordinator crashed
	if (currentAALSpace != null && !peers.keySet()
		.contains(currentAALSpace.getSpaceCard().getCoordinatorID())) {
	...
	final Map<String, Serializable> result = controlBroker
		.requestPeerAttributes(attributes, target, limit,
			(int) timeout);
	return result;
    }
