...
//							}
	...
	public void handle(Message msg, String senderID) {
		...
		case MessageType.EVENT:
			// EVENT type is for resending UIRequests and/or adaptation parameter changes
		...
		case MessageType.P2P_EVENT:
			...
			// this happens when the message content is a "parametrized" one, with the user input as the parameter
			...
			if (parametrizedNotification) {
				...
				if (input != null) {
					// the coordinator is telling me there is input for a caller; let's see if the caller is on this node
					UICaller caller = (UICaller) pendingRequests.get(input
							.getDialogID());
					if (caller != null) {
						// zes, the caller is a member of this instance, so I must notify it
						caller.handleUIResponse(input);
						return;
					}
				}
			...
			// the next priority is to wake up all threads waiting for the discovery of the coordinator instance
			// handle the bus message that indicates the bus-coordinator: is it a question or an announcement
			if (res.getType().equals(TYPE_uAAL_UI_BUS_COORDINATOR)) {
				// this should be a notification from the side of the coordinator announcing its role
				// => this is the announcement
				if (dialogManager == null // I am not the coordinator
						&& theCoordinator == null // I do not know the coordinator
// this check is probably not needed at all
//						&& res.getURI().startsWith(
//								Constants.uAAL_MIDDLEWARE_LOCAL_ID_PREFIX)
								) {
					// oh, I didn't know who is the coordinator => store this info
					synchronized (this) {
			...
			if (isCoordinator()) {
				// the coordinator instance might receive different notifications from the other instances
				...
				// This is in case we are not the coordinator & we have a parametrized notification other than user input that was in the beginning
				String handlerID = (String) res
						.getProperty(PROP_uAAL_UI_HANDLER_ID);
	...
	/**
	 * 
	 * Handle output-notifications. Look for {@link UIHandler} for relay the {@link UIRequest}.
	 * 
	 * @param handlerID
	 ...
	 * Also called by the {@link DialogManager} when the user has instructed to resume a
	 * specific dialog; Or when the {@link DialogManager} wants to resume a previously suspended
	 * dialog because of higher priority dialog interrupted.
	 * 
	 * @param dialogID
