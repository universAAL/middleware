...
public class UIHandlerProfile extends FinalizedResource implements UtilityAdvertisement {
	...
	public boolean addRestriction(MergedRestriction mergedRestriction) {
		...
		if (UIRequest.PROP_ADDRESSED_USER.equals(prop) || UIRequest.PROP_PRESENTATION_MODALITY.equals(prop)
				|| UIRequest.PROP_PRESENTATION_LOCATION.equals(prop)
				|| UIRequest.PROP_HAS_ACCESS_IMPAIRMENT.equals(prop) || UIRequest.PROP_DIALOG_PRIVACY_LEVEL.equals(prop)
				|| UIRequest.PROP_DIALOG_LANGUAGE.equals(prop) || UIRequest.PROP_DIALOG_FORM.equals(prop)
				|| UIRequest.PROP_DIALOG_PRIORITY.equals(prop) || UIRequest.PROP_HAS_PREFERENCE.equals(prop)) {
		...
		for (MergedRestriction r : restrictions) {
			if (r.hasMember(uiRequest)) {
				...
				if (r.getOnProperty().equals(UIRequest.PROP_ADDRESSED_USER)
						&& (uiRequest.getProperty(UIRequest.PROP_ADDRESSED_USER) != null)) {
					/*
				...
				if (r.getOnProperty().equals(UIRequest.PROP_PRESENTATION_MODALITY)
						&& (uiRequest.getProperty(UIRequest.PROP_PRESENTATION_MODALITY) != null)) {
					result += MATCH_MAIN_MODALITY;
				}
				// ALT MODALITY is checked later.
				if (r.getOnProperty().equals(UIRequest.PROP_PRESENTATION_LOCATION)
						&& (uiRequest.getProperty(UIRequest.PROP_PRESENTATION_LOCATION) != null)) {
					result += MATCH_USER_LOCATION;
				}
				if (r.getOnProperty().equals(UIRequest.PROP_HAS_ACCESS_IMPAIRMENT)
						&& (uiRequest.getProperty(UIRequest.PROP_HAS_ACCESS_IMPAIRMENT) != null)) {
					result += MATCH_USER_IMPAIRMENTS;
				}
				if (r.getOnProperty().equals(UIRequest.PROP_DIALOG_PRIVACY_LEVEL)
						&& (uiRequest.getProperty(UIRequest.PROP_DIALOG_PRIVACY_LEVEL) != null)) {
					result += MATCH_DIALOG_PRIVACY;
				}
				if (r.getOnProperty().equals(UIRequest.PROP_DIALOG_LANGUAGE)
						&& (uiRequest.getProperty(UIRequest.PROP_DIALOG_LANGUAGE) != null)) {
				...
				if (UIRequest.PROP_PRESENTATION_MODALITY.equals(r.getOnProperty())
						&& r.copyOnNewProperty(UIRequest.PROP_PRESENTATION_MODALITY_ALT).hasMember(uiRequest))
				/*
	...
	public boolean isClosedCollection(String propURI) {
		return !TypeExpression.PROP_RDFS_SUB_CLASS_OF.equals(propURI) && super.isClosedCollection(propURI);
	}
	...
	public boolean setSupportedInputModalities(Modality[] modalities) {
		...
		if (propRestrictionAllowed(UIRequest.PROP_PRESENTATION_MODALITY)) {
			MergedRestriction mr = MergedRestriction.getAllValuesRestriction(UIRequest.PROP_PRESENTATION_MODALITY,
					new Enumeration(modalities));
