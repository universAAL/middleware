...
public class UIBusOntology extends Ontology {
	...
	public static final String NAMESPACE = Resource.uAAL_NAMESPACE_PREFIX + "UIBus.owl#";
	...
	public void create() {
		...
		oci.setResourceComment(
				"General concept for representing impairments of the users in accessing the uAAL system.");
		oci.setResourceLabel("Access Impairment");
		oci.addSuperClass(ManagedIndividual.MY_URI);
		oci.addObjectProperty(AccessImpairment.PROP_IMPAIRMENT_LEVEL).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				AccessImpairment.PROP_IMPAIRMENT_LEVEL, LevelRating.MY_URI, 1, 1));
		...
		oci.addSuperClass(ManagedIndividual.MY_URI);
		oci.toEnumeration(new ManagedIndividual[] { DialogType.sysMenu, DialogType.message, DialogType.subdialog,
		...
		oci.addSuperClass(ManagedIndividual.MY_URI);
		oci.toEnumeration(new ManagedIndividual[] { Modality.voice, Modality.gui, Modality.gesture, Modality.sms,
				Modality.web, Modality.mobile });
		...
		oci.toEnumeration(new ManagedIndividual[] { PrivacyLevel.personal, PrivacyLevel.homeMatesOnly,
				PrivacyLevel.intimatesOnly, PrivacyLevel.knownPeopleOnly, PrivacyLevel.insensible });
