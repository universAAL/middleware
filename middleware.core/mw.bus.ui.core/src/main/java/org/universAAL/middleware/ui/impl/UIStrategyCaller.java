 ...
 * {@link UICaller}s and the {@link IDialogManager}. <center>
 * <img style="background-color:white;" src="doc-files/UIStrategyCaller.png" alt
	...
	void resumeDialog(String dialogID, Resource dialogData) {
		if (iAmCoordinator()) {
			...
			if (uiRequest != null) {
				...
				adaptationParametersChanged(dialogManager, uiRequest, (String) null);
			} else {
	...
	private void handleUIRequest(BusMessage message, String senderID) {
		...
		if (!message.senderResidesOnDifferentPeer()) {
			...
			if (form != null) {
				...
				LogUtils.logError(busModule, getClass(), "handleUIRequest",
						new Object[] { "Dialog form of the UIRequest could not be determined! Not allowed!" }, null);
				return;
			}
		}
		if (iAmCoordinator()) {
			...
			if (dialogManager.checkNewDialog(request)) {
				...
				LogUtils.logDebug(busModule, getClass(), "askDialogManagerForPresentation",
			...
			LogUtils.logError(busModule, getClass(), "handleUIRequest",
					new Object[] { "combination non-coordinator + remote. We shouldn't get here!!" }, null);
		}
	}
