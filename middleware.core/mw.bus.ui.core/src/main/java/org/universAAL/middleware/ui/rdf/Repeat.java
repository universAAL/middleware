...
public class Repeat extends Group {
	...
	public boolean addValue() {
		...
		if (selection != null && !values.contains(selection) && (selectionIndex == -1 || valuesDiffer())) {
	...
	public Object getValue(String[] pp) {
		return (pp == null || pp.length == 0) ? selection
				: (selection instanceof Resource) ? Form.getValue(pp, (Resource) selection) : null;
	}
	...
	boolean hasValue(String[] pp) {
		...
		for (Iterator i = values.iterator(); i.hasNext();) {
			Object o = i.next();
			if (o instanceof Resource && Form.getValue(pp, (Resource) o) != null)
				return true;
		}
	...
	boolean setValue(String[] pp, Object value, MergedRestriction valueRestrictions) {
		...
		if (selection instanceof Resource)
			return Form.setValue((Resource) selection, pp, value, valueRestrictions);
			...
			if (value == null) {
				...
				String prop = (aux instanceof PropertyPath) ? ((PropertyPath) aux).getLastPathElement() : null;
				...
				if (valueRestrictions != null && !valueRestrictions.hasMemberIgnoreCardinality(dummy))
					return false;
				selection = value;
				return true;
			}
	...
	private boolean valuesDiffer() {
		if (selection instanceof Resource) {
			Object o = values.get(selectionIndex);
			for (Enumeration e = ((Resource) selection).getPropertyURIs(); e.hasMoreElements();) {
				String key = e.nextElement().toString();
				if (!((Resource) selection).getProperty(key).equals(((Resource) o).getProperty(key)))
					return true;
			}
			return false;
		} else
				...
				if (elems[j] != null) {
					...
					if (elems[j] instanceof SubdialogTrigger) {
						nFC.changeProperty(SubdialogTrigger.PROP_SUBMISSION_ID,
								nFC.getProperty(SubdialogTrigger.PROP_REPEATABLE_ID_PREFIX) + Integer.toString(index));
					} else {
	...
	private static class VirtualForm extends Form {
		...
		static public Form createNewVirtualForm(Object root) {
			...
			new Group(rootg, new Label(Group.STD_IO_CONTROLS, null), null, null, null);
			return f;
		}
