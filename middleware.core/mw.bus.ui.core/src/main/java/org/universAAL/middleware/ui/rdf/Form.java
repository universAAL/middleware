 ...
 * <dd>"buttons" that finish the whole dialog should be added to this
 * group.</dd>
 ...
 * buttons can be used by a user to state either "ok, I got it, you can delete
 * the message" or "please preserve the message for later check", respectively.
 * Normally, applications do not need to subscribe for user decision in this
 * regard, but if they want, then they must use the value returned by the method
 * {@link #getDialogID()} of the created form object, for subscribing to the UI
 * bus.</dd>
 * <dt>System Menu</dt>
...
public class Form extends FormElement {
	...
	static MergedRestriction getPPathRestriction(String[] pp, Resource pr) {
		...
		return ManagedIndividual.getClassRestrictionsOnProperty(pr.getType(), pp[pp.length - 1]);
	}

	static MergedRestriction getPPathRestriction(String[] pp, int i, String typeURI) {
		if (typeURI == null)
			return null;

		MergedRestriction r = ManagedIndividual.getClassRestrictionsOnProperty(typeURI, pp[i]);
		if (i == pp.length - 1)
			return r;

		return (r == null) ? null : getPPathRestriction(pp, i + 1, r.getPropTypeURI());
	}
	...
	public static Form newMessage(String formTitle, String message) {
		...
		Group root = (Group) f.props.get(PROP_ROOT_GROUP);
		Group ctrls = new Group(root, new Label(Group.STD_IO_CONTROLS, null), null, null, null);
		new SimpleOutput(ctrls, null, null, message);
		Group submits = new Group(root, new Label(Group.STD_SUBMITS, null), null, null, null);
	...
	static boolean setValue(Resource pr, String[] pp, Object value, MergedRestriction valueRestrictions) {
	...
	private Form(String formTitle, Resource dataRoot) {
		...
		props.put(PROP_DIALOG_DATA_ROOT, (dataRoot == null) ? new Resource() : dataRoot);
	}
	...
	public Group getIOControls() {
		FormControl[] children = getRootGroup().getChildren();
		for (int i = 0; i < children.length; i++)
			if (children[i] instanceof Group && Group.STD_IO_CONTROLS.equals(children[i].getLabel().getText()))
	...
	public Resource getParentDialogResource() {
		return isSubdialog() ? (Resource) props.get(PROP_PARENT_DIALOG_URI) : null;
	}
	...
	public Group getStandardButtons() {
		FormControl[] children = getRootGroup().getChildren();
		for (int i = 0; i < children.length; i++)
			if (children[i] instanceof Group && Group.STD_STD_BUTTONS.equals(children[i].getLabel().getText()))
				return (Group) children[i];
		return null;
	}
	...
	public Group getSubmits() {
		FormControl[] children = getRootGroup().getChildren();
		for (int i = 0; i < children.length; i++)
			if (children[i] instanceof Group && Group.STD_SUBMITS.equals(children[i].getLabel().getText()))
				return (Group) children[i];
		return null;
	}
