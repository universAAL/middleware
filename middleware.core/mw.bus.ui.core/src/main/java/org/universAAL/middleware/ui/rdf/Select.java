...
public class Select extends Input {
	 ...
	 *            in the form data.
	 */
	public Select(Group parent, Label label, PropertyPath ref, MergedRestriction valueRestriction,
			Object initialValue) {
		super(MY_URI, parent, label, ref, valueRestriction, initialValue);
	}

	protected Select(String typeURI, Group parent, Label label, PropertyPath ref, MergedRestriction valueRestriction,
			Object initialValue) {
	...
	public boolean generateChoices(Object[] elems) {
		...
		for (int i = 0; i < elems.length; i++) {
			if (elems[i] == null)
				return false;
			String label = (elems[i] instanceof Resource) ? ((Resource) elems[i]).getResourceLabel() : null;
			if (label == null)
				label = elems[i].toString();
			addChoiceItem(new ChoiceItem(StringUtils.deriveLabel(label), null, elems[i]));
		}
