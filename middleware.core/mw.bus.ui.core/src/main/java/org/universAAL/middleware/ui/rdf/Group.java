...
public class Group extends FormControl {
	 ...
	 *            this value.
	 */
	public Group(Group parent, Label label, PropertyPath ref, MergedRestriction valueRestriction,
			Resource initialValue) {
		super(MY_URI, parent, label, ref, valueRestriction, initialValue);
	}

	protected Group(String typeURI, Group parent, Label label, PropertyPath ref, MergedRestriction valueRestriction,
			Object initialValue) {
	...
	public void doModelBasedExpansion() {
		String t = getTypeURI();
		if (t == null) {
			LogUtils.logWarn(UIBusImpl.getModuleContext(), Group.class, "doModelBasedExpansion",
					new Object[] { "missing model ref!" }, null);
			return;
		}

		String[] props = OntologyManagement.getInstance().getOntClassInfo(t).getDeclaredPropertyURIs();
		...
		for (int i = 0; i < props.length; i++) {
			MergedRestriction r = ManagedIndividual.getClassRestrictionsOnProperty(t, props[i]);
			if (r == null) {
				LogUtils.logWarn(UIBusImpl.getModuleContext(), Group.class, "doModelBasedExpansion",
						new Object[] { "ignoring a property with unknown restrictions: ", props[i] }, null);
				continue;
			}
			...
			if (values == null || values.length == 0) {
				...
				if (tt == null)
					LogUtils.logWarn(UIBusImpl.getModuleContext(), Group.class, "doModelBasedExpansion",
							new Object[] { "ignoring a property with insufficient restrictions: ", props[i] }, null);
				else if (OntologyManagement.getInstance().isRegisteredClass(tt, true)) {
					Group g = new Group(this, new Label(StringUtils.deriveLabel(props[i]), null),
							constructSubpath(props[i]), null, null);
					g.doModelBasedExpansion();
				} else if (TypeMapper.isRegisteredDatatypeURI(tt))
					new InputField(this, new Label(StringUtils.deriveLabel(props[i]), null), constructSubpath(props[i]),
							null, null);
				else
					LogUtils.logWarn(UIBusImpl.getModuleContext(), Group.class, "doModelBasedExpansion",
							new Object[] { "ignoring a property with unknown type: ", props[i] }, null);
			} else {
				Select s = (r.getMinCardinality() > 1 || r.getMaxCardinality() != 1)
						? new Select(this, new Label(StringUtils.deriveLabel(props[i]), null),
								constructSubpath(props[i]), null, null)
						: new Select1(this, new Label(StringUtils.deriveLabel(props[i]), null),
								constructSubpath(props[i]), null, null);
	...
	String getTypeURI(String[] pp) {
		...
		if (r instanceof MergedRestriction && mypp instanceof PropertyPath
			...
			MergedRestriction target = ((MergedRestriction) r).getRestrictionOnPath(subpath);
			if (target != null) {
	...
	void setStructuralProps(int level) {
		...
		if (complexity[LevelRating.FULL] > 0 || complexity[LevelRating.HIGH] > 0 || complexity[LevelRating.MIDDLE] > 3)
			this.complexity = LevelRating.full;
		else if (complexity[LevelRating.LOW] > 3 || complexity[LevelRating.MIDDLE] > 0)
			this.complexity = LevelRating.high;
		else if (complexity[LevelRating.NONE] > 3 || complexity[LevelRating.LOW] > 0)
