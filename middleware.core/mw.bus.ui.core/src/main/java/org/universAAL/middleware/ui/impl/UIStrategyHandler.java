 ...
 * {@link UIHandler}s and the {@link IDialogManager}. <center>
 * <img style="background-color:white;" src="doc-files/UIStrategyHandler.png"
...
public abstract class UIStrategyHandler extends UIStrategyCoordinatorMng {
	...
	private class FinishDialogMessage extends Resource
			implements EventMessage<UIStrategyCaller>, IUIStrategyMessageSharedProps {
		/**
	...
	private class CutCallMessage extends CallMessage<UIStrategyHandler> implements IUIStrategyMessageSharedProps {
		...
		/** {@ inheritDoc} */
		protected void onResponse(UIStrategyHandler strategy, BusMessage m, String senderID) {
	...
	void adaptationParametersChanged(IDialogManager dm, UIRequest uiRequest, String changedProp) {
		if (dm != null && dm == dialogManager) {
			...
			String currentHandler = runningDialogs.getHandler(uiRequest.getDialogID());
			if (changedProp == null) {
				...
				if (currentHandler != null) {
					LogUtils.logWarn(busModule, getClass(), "adaptationParametersChanged",
							new Object[] { "strange situation: duplication dialog ID?\n", uiRequest }, null);
				}
			} else if (currentHandler == null) {
				LogUtils.logError(busModule, getClass(), "adaptationParametersChanged",
			...
			if (selectedHandler == null) {
				...
				LogUtils.logInfo(busModule, getClass(), "adaptationParametersChanged",
						new Object[] { "No UI Handler could be selected so dialog is suspended for now.", uiRequest },
						null);
				return;
			}
			...
			if (selectedHandler.equals(currentHandler)) {
				...
				// changed.
				if (changedProp != null) {
					notifyHandler_apChanged(currentHandler, uiRequest, changedProp);
					return;
				}
			}
	...
	protected Resource cutDialog(String handlerID, String dialogID) {
		...
		if (bm instanceof UIHandler) {
			...
			try {
				...
				LogUtils.logError(busModule, getClass(), "CutDialog", "Cut Call to move dialog was aborted.");
			}
		}
		return null;
	}
	...
	void dialogFinished(final String handlerID, final UIResponse input) {
		if (input == null) {
			LogUtils.logWarn(busModule, getClass(), "dialogFinished",
					new Object[] { "Dialog is finished by the user but UI Handler sent empty UI Response!" }, null);
			return;
		}
		// first handle the bus internal handling of this request
		if (iAmCoordinator()) {
			// do it in a new thread to make sure that no deadlock will happen
			new Thread(new DialogFinishedTask(handlerID, input), "UI Bus Strategy - Handling dialog finished").start();
		} else {
			// send message
	...
	private String selectHandler(UIRequest uiRequest) {
			...
			LogUtils.logDebug(busModule, getClass(), "selectHandler",
					new Object[] {
							"\n START+++++++++++++++++++++++++++++++++++++++++++++++\n Handler with subscription id: "
									+ profId + "\nhas matching degree: " + tempMatchingDegree + " \n in binary is: \n  "
									+ String.format("%8s", Integer.toBinaryString(tempMatchingDegree)).replace(' ', '0')
											.replaceAll("(\\d)", "$1\t")
									...
									+ uiRequest.getProperty(UIRequest.PROP_PRESENTATION_MODALITY)
									+ "\n Presentation location: " + uiRequest.getPresentationLocation()
									+ "\n STOP+++++++++++++++++++++++++++++++++++++++++++++++\n " },
					null);

			if (tempMatchingDegree > UIHandlerProfile.MATCH_LEVEL_FAILED) {

				if (profId.equals(lastUsedHandler.get(uiRequest.getAddressedUser().getURI()))) {
		...
		LogUtils.logDebug(busModule, getClass(), "selectHandler",
				new Object[] { "Handler with id: " + selectedHandler + ", and matching degree: " + maxMatchDegree
	 ...
	 */
	private void notifyHandler_apChanged(String handlerID, UIRequest request, String changedProp) {
		...
		if (o instanceof UIHandler) {
			...
			if (changedProp != null) {
				((UIHandler) o).adaptationParametersChanged(request.getDialogID(), changedProp,
			...
			LogUtils.logWarn(busModule, getClass(), "notifyHandler_apChanged",
	...
	private void notifyHandler_handle(String handlerID, UIRequest request) {
		...
		if (o instanceof UIHandler) {
			...
			sendEventToRemoteBusMember(handlerID, new NotifyHandlerMessage(handlerID, request));
		} else
			LogUtils.logWarn(busModule, getClass(), "notifyHandler_handle", new Object[] {
					"Unpredicted situation happened while handling a uiRequest-notification!", o.getClass().getName() },
					null);
	}
