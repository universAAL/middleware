 ...
 *
 */
public class OntAutoLoader implements ModuleActivator {

	private static File owlDir = null;
	private ModuleContext mc;
	private MessageContentSerializer contentSerializer;
	private List<OntologyLoaderTask> loadingOrder;
	...
	private class OntologyLoaderTask implements LogListener {
		...
		int warnings = 0;
		int errors = 0;
		List<LogEntry> logEntries = new ArrayList<OntAutoLoader.LogEntry>();
	...
	public void start(ModuleContext mc) {
		this.mc = mc;
		mc.logInfo("OntAutoLoader", "Auto Loading Ontologies.", null);
		owlDir = mc.getDataFolder();
		...
		Map<Ontology, OntologyLoaderTask> pendingOnts = new HashMap<Ontology, OntAutoLoader.OntologyLoaderTask>();
		loadingOrder = new ArrayList<OntAutoLoader.OntologyLoaderTask>();
		...
		mc.logInfo("OntAutoLoader", "Auto Loading complete.", null);
	}
	...
	private void TryLoadingOnts(List<Ontology> toBeLoaded,
		...
		while (!ontStep.isEmpty()) {
			...
			otl.attempt();
			if ((!otl.allImportsRegistered() || otl.errors > 0)
	...
	/**
	 * This is not a testing project thus this always returns false. This method
	 * is inherited from OntTestCase.
	 * 
	 * @param ont
