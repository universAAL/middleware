...
public class FileUtils {
	...
	public static File createFileFromByte(ModuleContext mc, byte[] content, String dst, boolean overwrite) {
		...
		if (file.exists() == true && file.isDirectory() == true) {
			LogUtils.logError(mc, FileUtils.class, METHOD, new Object[] {
					"Error while creating file the destination " + file.getPath() + " exists but it is a directory" },
					null);
			return null;
		}
		if (file.exists() && overwrite == true && file.delete() == false) {
			LogUtils.logError(mc, FileUtils.class, METHOD, new Object[] {
					"Error while creating file the destination " + file.getPath() + " exists but couldn't delete it" },
					null);
			return null;
		...
		if (parent == null) {
			LogUtils.logDebug(mc, FileUtils.class, METHOD,
			...
			if (parent.exists() == true && parent.isDirectory() == false) {
				LogUtils.logError(mc, FileUtils.class, METHOD,
						new Object[] { "Error while creating file the destination folder " + parent.getPath()
