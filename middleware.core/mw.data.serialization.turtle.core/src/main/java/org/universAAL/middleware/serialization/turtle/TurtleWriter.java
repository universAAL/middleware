...
public final class TurtleWriter {
	...
	static String serialize(Object o, int embedLevel) {
		if (!(o instanceof Resource || o instanceof Ontology)) {
			LogUtils.logError(TurtleUtil.moduleContext, TurtleWriter.class, "serialize",
					new Object[] { "Cannot serialize objects other than instances of Resource or Ontology!" }, null);
			return null;
		}
		...
		try {
			...
			LogUtils.logError(TurtleUtil.moduleContext, TurtleWriter.class, "serialize", null, e);
			return null;
		}
	}
	...
	private void analyzeObject(Object o, Hashtable nsTable, int reduction) {
		...
		else if (!(o instanceof BigInteger) && !(o instanceof Boolean) && !(o instanceof Double)
	...
	private void analyzeResource(Resource r, Hashtable nsTable) {
		...
		for (Enumeration e = r.getPropertyURIs(); e.hasMoreElements();) {
			...
			int reduction = isRDFType ? Resource.PROP_SERIALIZATION_REDUCED : r.getPropSerializationType(prop);
			...
			if (o instanceof List) {
				...
				if (isRDFType) {
					...
					aux.add(o);
					String[] types = ManagedIndividual.getNonabstractSuperClasses(o.toString());
					if (types != null)
						for (int j = 0; j < types.length; j++) {
	 ...
	 */
	private static synchronized String createUniqueLocalID(String localPrefix, int counter, int counterLen) {
	...
	private void handleStatement(Resource subj, String pred, List types, boolean force) throws IOException {
		...
		int redType = (obj instanceof List) ? getListReduction(subj, pred, (List) obj)
				: (obj instanceof Resource && !((Resource) obj).serializesAsXMLLiteral())
						? getData((Resource) obj).redType : subj.getPropSerializationType(pred);
	 ...
	 * @throws IOException
	 */
	private void writeLiteral(String lexicalForm, String datatype) throws IOException {
		if (lexicalForm.indexOf('\n') > -1 || lexicalForm.indexOf('\r') > -1 || lexicalForm.indexOf('\t') > -1) {
		...
		if (d == null) {
			...
			if (descriptionClosed && d.serialized != NOT_SERIALIZED) {
				if (res.isAnon() && d.refs < 0)
					...
					throw new RuntimeException("Attempt to serialize " + res + " twice!");
			}

			if (d.refs < 0)
				throw new RuntimeException("Resource " + res + " has no open references!");
	...
	private void writeValue(Object val, boolean closed, boolean isType) throws IOException {
		if (val instanceof List) {
			...
			if (((Resource) val).serializesAsXMLLiteral()) {
				if (((Resource) val).isAnon() || ((Resource) val).numberOfProperties() > 0) {
					writeLiteral(TurtleWriter.serialize(val, embedLevel + 2), TurtleUtil.xmlLiteral);
				} else {
					writeLiteral(((Resource) val).getURI(), TypeMapper.getDatatypeURI(Resource.class));
				}
			} else {
			...
			// also the possibility to force a formatting (should we do
			// this?)
			if (strval.indexOf('e') == -1 && strval.indexOf('E') == -1 && strval.indexOf('.') >= 0) {
				writer.write(strval);
				return;
			}
			// else: use standard formatting with lexical form and datatype
