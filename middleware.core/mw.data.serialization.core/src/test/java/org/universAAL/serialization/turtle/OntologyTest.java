...
public class OntologyTest extends TestCase {
	...
	public void testOntology() {
		...
		final class LightingOntology extends Ontology {
			...
			public void create() {
				...
				oci.toEnumeration(
						new ManagedIndividual[] { new GenericManagedIndividual(ns + "NaturalLightMY_URI", "moonShine"),
								new GenericManagedIndividual(ns + "NaturalLightMY_URI", "sunShine"), });

				// load LightSource
				oci = createNewOntClassInfo(ns + "LightSourceMY_URI", factory, 0);
				...
				oci.addInstance(new GenericManagedIndividual(ns + "LightSourceMY_URI", "myLightSource"));
			}
		}
	...
	private String getTriple(GraphIteratorElement el) {
		...
		if (el.getObject() instanceof Resource && ((Resource) el.getObject()).isAnon())
			s += "Anon";
		else
