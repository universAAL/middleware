...
public class ManifestTestCase extends BusTestCase {
	...
	protected void writeManifest() {
		...
		writeManifestEntry(writer, "mw.bus.service", serviceRequests, serviceProfiles);
		writeManifestEntry(writer, "mw.bus.context", contextEventPatternsSubscribe, contextEventPatternsPublish);
	...
	private void writeManifestEntry(PrintWriter writer, String brokerName, LinkedList elReq, LinkedList elAd) {
	...
	private void writeManifestEntry(PrintWriter writer, String type, LinkedList<Element> elements) {
		if (elements != null)
	...
	private void writeManifestEntry(PrintWriter writer, String type, Element el) {
		...
		writer.println("            <description>" + el.description + "</description>");
	...
	protected void add(String title, String description, ContextEventPattern cep, boolean isPublisher) {
		LogUtils.logDebug(mc, ManifestTestCase.class, "add", new Object[] { "Adding manifest entry: ", title }, null);
	...
	protected void add(String title, String description, UIHandlerProfile profile) {
		...
		uiHandler.add(new Element<UIHandlerProfile>(title, description, profile));
	}
	...
	protected void add(String title, String description, ServiceProfile profile) {
		...
		assertTrue(profile.matches(profile));
		checkserialization(profile);
		serviceProfiles.add(new Element<ServiceProfile>(title, description, profile));
	}

	protected ServiceRequest add(String title, String description, ServiceRequest r, boolean extendAll) {
		return add(title, description, r, extendAll ? EXTEND_ALL : EXTEND_NOTHING);
	}
	...
	private boolean extendRequestChangeEffect(GraphIteratorElement el) {
		...
		if (sub instanceof Resource) {
			...
			if (ProcessEffect.TYPE_PROCESS_CHANGE_EFFECT.equals(r.getType())) {
				if (extendRequestTrySettingProcessParameter(r, ProcessEffect.PROP_PROCESS_PROPERTY_VALUE))
					return true;
			}
		}
		return false;
	...
	private boolean extendRequestTrySettingProcessParameter(Resource r, String propURI) {
		...
		if (!(obj instanceof ProcessParameter)) {
			...
			if (typeURI != null) {
				...
				ProcessParameter in = new ProcessParameter(null, ProcessParameter.MY_URI);
	...
	private boolean extendRequestHasValue(GraphIteratorElement el) {
		...
		if (sub instanceof HasValueRestriction) {
			...
			if (extendRequestTrySettingProcessParameter(has, HasValueRestriction.PROP_OWL_HAS_VALUE))
				return true;
		}
