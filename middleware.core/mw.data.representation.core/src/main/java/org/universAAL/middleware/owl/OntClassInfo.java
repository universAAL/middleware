...
public final class OntClassInfo extends RDFClassInfo {
	...
	private class PrivateOntSetup extends PrivateRDFSetup implements OntClassInfoSetup {
		...
		public void addRestriction(MergedRestriction r) {
			if (locked)
				return;
			if (r == null)
				throw new NullPointerException("The restriction must be not null.");

			r = (MergedRestriction) r.copy();

			if (propRestriction.containsKey(r.getOnProperty()))
				// a restriction for this property already exists
				throw new IllegalAccessError("A restriction for this property (" + r.getOnProperty()
		...
		/** {@inheritDoc} */
		public void addInstance(ManagedIndividual instance) throws UnsupportedOperationException {
			if (isEnumeration)
			...
			if (instance != null && uri.equals(instance.getClassURI())) {
				...
				if (instance == null) {
					LogUtils.logDebug(SharedResources.moduleContext, OntClassInfo.class, "addInstance",
							new Object[] { "An instance of the ontology class ", uri,
									" should be added but the given instance is null." },
							null);
				} else {
					LogUtils.logDebug(SharedResources.moduleContext, OntClassInfo.class, "addInstance",
							new Object[] { "The instance ", instance.getURI(), " of the ontology class ", uri,
	...
	public final class ManagementOperation {
		...
		public void removeExtender(OntClassInfo extenderInfo) {
			...
			// all extenders
			OntClassInfo newCombinedInfo = new OntClassInfo(getURI(), ont, factory, factoryIndex);
	 ...
	 *            ignored.
	 */
	private OntClassInfo(String classURI, Ontology ont, ResourceFactory factory, int factoryIndex) {
	...
	public static RDFClassInfoSetup create(String classURI, Ontology ont, ResourceFactory factory, int factoryIndex) {
		...
		OntClassInfo info = new OntClassInfo(classURI, ont, factory, factoryIndex);
		return info.setup;
	}
	...
	public ManagementOperation getManagementOperation(String permissionCheck) {
		if (!OntologyManagement.getInstance().checkPermission(permissionCheck))
			throw new IllegalAccessError("The management operations can only be called from OntologyManagement!");
