...
public final class MergedRestriction extends Intersection {
	 ...
	 */
	public MergedRestriction(String onProperty, ArrayList<PropertyRestriction> restrictions) {
		this();
	 ...
	 *         invalid.
	 */
	public static MergedRestriction getFixedValueRestriction(String propURI, Object value) {
	 ...
	 *         invalid.
	 */
	public static MergedRestriction getAllValuesRestrictionWithCardinality(String propURI, String typeURI, int min,
			int max) {
		if (typeURI == null)
			return null;
		...
		else if (OntologyManagement.getInstance().isRegisteredClass(typeURI, true))
			type = new TypeURI(typeURI, false);

		if (type == null) {
			LogUtils.logDebug(SharedResources.moduleContext, MergedRestriction.class,
					"getAllValuesRestrictionWithCardinality",
					new String[] { "Can not create the restriction because of invalid input parameters: "
							+ "the specified type URI (" + typeURI
	 ...
	 *         invalid.
	 */
	public static MergedRestriction getAllValuesRestrictionWithCardinality(String propURI, TypeExpression expr, int min,
			int max) {
		if (expr == null)
			return null;
		...
		MergedRestriction ret = new MergedRestriction(propURI);
		if (min > 0 || max >= 0)
			ret.addRestriction(MergedRestriction.getCardinalityRestriction(propURI, min, max));
	 ...
	 *         invalid.
	 */
	public static MergedRestriction getAllValuesRestriction(String propURI, String typeURI) {
		...
		if (type == null) {
			LogUtils.logDebug(SharedResources.moduleContext, MergedRestriction.class, "getAllValuesRestriction",
					new String[] { "Can not create the restriction because of invalid input parameters: "
							+ "the specified type URI (" + typeURI
							+ ") is not registered. It is neither a data type nor a registered ontology class." },
					null);
	 ...
	 *         invalid.
	 */
	public static MergedRestriction getAllValuesRestriction(String propURI, TypeExpression expr) {
	...
	public int getMinCardinality() {
		if (index[minCardinalityID] != -1)
			return ((MinCardinalityRestriction) (types.get(index[minCardinalityID]))).getValue();
		if (index[exactCardinalityID] != -1)
			return ((ExactCardinalityRestriction) (types.get(index[exactCardinalityID]))).getValue();
	...
	public int getMaxCardinality() {
		if (index[maxCardinalityID] != -1)
			return ((MaxCardinalityRestriction) (types.get(index[maxCardinalityID]))).getValue();
		if (index[exactCardinalityID] != -1)
			return ((ExactCardinalityRestriction) (types.get(index[exactCardinalityID]))).getValue();
	 ...
	 *            instances of {@link PropertyRestriction}.
	 */
	private void setRestrictions(String onProperty, ArrayList<PropertyRestriction> restrictions) {
	...
	public boolean addRestrictionCheck(PropertyRestriction res) {
		...
		if (max == 0) {
			LogUtils.logDebug(SharedResources.moduleContext, MergedRestriction.class, "addRestriction",
					new String[] { "Can not add the PropertyRestriction (" + res.getType()
		...
		case allValuesFromID:
			if (all == null
					&& (some == null || (max != 1 && res.matches(some, null, getDefaultMatchmakingTTL(), null)))) {
				...
				if (all != null)
					LogUtils.logDebug(SharedResources.moduleContext, MergedRestriction.class, "addRestriction",
							new String[] {
									"Can not add the AllValuesFromRestriction because such a restriction is already set." },
							null);
				else
					LogUtils.logDebug(SharedResources.moduleContext, MergedRestriction.class, "addRestriction",
							new String[] {
									"Can not add the AllValuesFromRestriction, please check with your SomeValuesFromRestriction." },
							null);
			}
			break;
		case someValuesFromID:
			if (some == null && (all == null || ((TypeExpression) all).matches(res))) {
				if (all != null && max == 1)
		...
		case hasValueID:
			...
			if (has == null) {
				...
				LogUtils.logDebug(SharedResources.moduleContext, MergedRestriction.class, "addRestriction",
						new String[] {
								"Can not add the HasValueRestriction because such a restriction is already set." },
						null);
			}
			break;
		...
		case maxCardinalityID:
			...
			if (max == MAX_UNDEFINED && (newMax > 1 || (newMax == 1 && (some == null || all == null))
					|| (newMax == 0 && all == null && some == null)))
				if (min < newMax) {
		...
		case exactCardinalityID:
			...
			if (max == MAX_UNDEFINED && getRestriction(minCardinalityID) == null
					&& (newExact > 1 || (newExact == 1 && (some == null || all == null))
							|| (newExact == 0 && all == null && some == null)))
				if (index[exactCardinalityID] == -1) {
					index[exactCardinalityID] = types.size();
					types.add(res);
					return true;
	...
	public MergedRestriction addRestriction(MergedRestriction r) {
		...
		for (int i = 0; i < resList.size(); i++)
			addRestriction((PropertyRestriction) ((TypeExpression) resList.get(i)).copy());
		return this;
	}
	...
	private int getID(PropertyRestriction res) {
		...
		if (res instanceof AllValuesFromRestriction)
			...
			throw new IllegalArgumentException(
					"Unknown Restriction type: " + res + " (Class URI: " + res.getClassURI() + ")");
		return idx;
	}
	...
	private void analyze() {
		for (int i = 0; i < types.size(); i++) {
			if (!(types.get(i) instanceof PropertyRestriction))
				throw new IllegalArgumentException("Non-restriction found at index: " + i);

			PropertyRestriction res = (PropertyRestriction) types.get(i);
			if (!onProperty.equals(res.getOnProperty()))
				throw new IllegalArgumentException(
						"Restriction defined for wrong property: " + res.getClassURI() + " " + res.getOnProperty());

			int idx = getID(res);
			if (idx != -1 && index[idx] != -1)
				throw new IllegalArgumentException("Duplicate Restriction: " + res.getClassURI());
			index[idx] = i;
		}
	}
	...
	public MergedRestriction copyOnNewProperty(String onProp) {
		...
		for (Iterator<TypeExpression> i = types.iterator(); i.hasNext();)
			((Resource) i.next()).setProperty(PropertyRestriction.PROP_OWL_ON_PROPERTY, onProp);
		onProperty = onProp;
		return r;
	}
	...
	public String getPropTypeURI() {
		...
		all = (TypeExpression) ((AllValuesFromRestriction) types.get(index[allValuesFromID])).getConstraint();
	...
	public MergedRestriction appendTo(MergedRestriction root, String[] path) {
		...
		if (path == null || path.length == 0) {
			LogUtils.logDebug(SharedResources.moduleContext, MergedRestriction.class, "appendTo",
					new String[] { "Not possible to append a restriction because the specified path is invalid: ",
							path == null ? "path is null." : "path is empty." },
					null);
			return null;
		}
		...
		if (all == null && types.size() == 1) {
			...
			tmp.changeProperty(AllValuesFromRestriction.PROP_OWL_ALL_VALUES_FROM, types.get(0));
		} else {
			...
			if (!(all instanceof Intersection)) {
				...
				if (all != null)
					i.addType(all);
				tmp.changeProperty(AllValuesFromRestriction.PROP_OWL_ALL_VALUES_FROM, i);
				all = i;
			}
	...
	private MergedRestriction getRestrictionOnPathElement(String pathElement) {
		...
		if (all instanceof Intersection) {
			...
			for (Iterator<TypeExpression> i = ((Intersection) all).types(); i.hasNext();) {
	...
	public Object[] getEnumeratedValues() {
		...
		if (idx < 0)
			return null;
		AllValuesFromRestriction allres = (AllValuesFromRestriction) types.get(idx);
		if (allres != null) {
			...
			if (all instanceof Enumeration)
				...
				OntClassInfo info = OntologyManagement.getInstance().getOntClassInfo(all.getURI());
	...
	public boolean hasMemberIgnoreCardinality(Object o) {
		int j = 0;
		for (Iterator<TypeExpression> i = types.iterator(); i.hasNext();) {
			if (j != minCardinalityID && j != maxCardinalityID && j != exactCardinalityID)
