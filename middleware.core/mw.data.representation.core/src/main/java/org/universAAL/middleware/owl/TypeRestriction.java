...
public abstract class TypeRestriction extends TypeExpression {
	...
	protected static final String XSD_FACET_PATTERN = TypeMapper.XSD_NAMESPACE + "pattern";
	...
	public boolean setProperty(String propURI, Object o) {
		...
		if (PROP_OWL_ON_DATATYPE.equals(propURI)) {
			if (o instanceof Resource && getTypeURI().equals(((Resource) o).getURI()))
				return true;
			return false;
	...
	protected Comparable resolveVarByGreaterEqual(Variable v, Comparable lowerbound, boolean canBeEqual,
			HashMap context) {
	...
	protected Comparable resolveVarByLessEqual(Variable v, Comparable upperbound, boolean canBeEqual, HashMap context) {
		Comparable resolution = canBeEqual ? upperbound : getPrevious(upperbound);
	...
	protected boolean hasMember(Object member, HashMap context, int ttl, List<MatchLogEntry> log, Object min,
		...
		if (!(valueToCheck instanceof Variable) && !(valueToCheck instanceof Comparable))
			return false;
		...
		if (context == null) {
			if (valueToCheck instanceof Variable || lowerBound instanceof Variable || upperBound instanceof Variable)
				return false;
		} else if (valueToCheck instanceof Variable && lowerBound instanceof Variable && upperBound instanceof Variable)
			return false;
		...
		if (upperBound instanceof Variable) {
			...
			if (valueToCheck instanceof Variable) {
				...
				valueToCheck = resolveVarByGreaterEqual((Variable) valueToCheck, (Comparable) lowerBound, minInclusive,
						cloned);
				...
				lowerBound = resolveVarByLessEqual((Variable) lowerBound, (Comparable) valueToCheck, minInclusive,
						cloned);
				if (lowerBound == null)
					// deadend
					return false;
			} else if (((Comparable) valueToCheck).compareTo(lowerBound) < 0
					|| (!minInclusive && ((Comparable) valueToCheck).compareTo(lowerBound) == 0))
			...
			upperBound = resolveVarByGreaterEqual((Variable) upperBound, (Comparable) valueToCheck, maxInclusive,
					cloned);
			if (upperBound == null)
				// deadend
				return false;
			...
			if (valueToCheck instanceof Variable) {
				...
				valueToCheck = resolveVarByLessEqual((Variable) valueToCheck, (Comparable) upperBound, maxInclusive,
						cloned);
				if (valueToCheck == null)
					// deadend
					return false;
			} else if (((Comparable) valueToCheck).compareTo(upperBound) > 0
					|| (!maxInclusive && ((Comparable) valueToCheck).compareTo(upperBound) == 0))
			...
			lowerBound = resolveVarByLessEqual((Variable) lowerBound, (Comparable) valueToCheck, minInclusive, cloned);
			if (lowerBound == null)
				// deadend
				return false;
	...
	protected boolean matches(Object other_min, boolean other_minInclusive, Object other_max,
			boolean other_maxInclusive, HashMap context, int ttl, List<MatchLogEntry> log, Object min,
			boolean minInclusive, Object max, boolean maxInclusive) {
		// ttl =
		checkTTL(ttl);
