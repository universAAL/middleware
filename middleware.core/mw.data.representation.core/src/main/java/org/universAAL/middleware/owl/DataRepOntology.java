	...
	public void create() {
		...
		try {
			...
			oci.setResourceComment(
					"An enumeration for specifying the level of appearance / availability of a phenomen.");
			oci.setResourceLabel("Level Rating");
			oci.addSuperClass(ComparableIndividual.MY_URI);
			oci.toEnumeration(new ManagedIndividual[] { LevelRating.none, LevelRating.low, LevelRating.middle,
					LevelRating.high, LevelRating.full });
			...
			oci.addSuperClass(ComparableIndividual.MY_URI);
			oci.toEnumeration(new ManagedIndividual[] { Rating.poor, Rating.almostPoor, Rating.almostSufficient,
					Rating.sufficient, Rating.richSufficient, Rating.almostSatisfying, Rating.satisfying,
					Rating.richSatisfying, Rating.almostGood, Rating.good, Rating.richGood, Rating.almostExcellent,
					Rating.excellent });

		} catch (Exception e) {// IllegalAccessException e) {
			e.printStackTrace();
			LogUtils.logDebug(SharedResources.moduleContext, this.getClass(), "create",
