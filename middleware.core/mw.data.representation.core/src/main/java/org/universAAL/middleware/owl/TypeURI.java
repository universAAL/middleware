...
public final class TypeURI extends TypeExpression {
	...
	public static TypeURI asTypeURI(Object obj) {
		...
		if (obj instanceof Resource && !((Resource) obj).isAnon()) {
			...
			else if (OntologyManagement.getInstance().isRegisteredClass(uri, true))
				return new TypeURI(uri, false);
		} else if (obj instanceof String)
			if (TypeMapper.isRegisteredDatatypeURI((String) obj))
				return new TypeURI((String) obj, true);
			else if (OntologyManagement.getInstance().isRegisteredClass((String) obj, true))
	...
	public Object[] getUpperEnumeration() {
		OntClassInfo info = OntologyManagement.getInstance().getOntClassInfo(getURI());
	...
	public boolean matches(TypeExpression subtype, HashMap context, int ttl, List<MatchLogEntry> log) {
		...
		if (subtype instanceof Enumeration)
			return ((Enumeration) subtype).hasSupertype(this, context, ttl, log);
		...
		if (subtype instanceof Intersection && !(subtype instanceof MergedRestriction)) {
					...
					if (Variable.isVarRef(o)) {
						...
						if (val instanceof Variable) {
							...
							if (var.getMinCardinality() < 2
									&& TypeMapper.getDatatypeURI(Resource.class).equals(var.getParameterType())) {
			...
			MergedRestriction r = ManagedIndividual.getClassRestrictionsOnProperty(uri,
	...
	public boolean isDisjointWith(TypeExpression other, HashMap context, int ttl, List<MatchLogEntry> log) {
		...
		if (other instanceof Complement)
			return ((Complement) other).getComplementedClass().matches(this, context, ttl, log);
		...
		if (other instanceof PropertyRestriction) {
			...
			return r != null && ((PropertyRestriction) other).isDisjointWith(r, context, ttl, log);
		} else if (other instanceof MergedRestriction) {
			MergedRestriction r = ManagedIndividual.getClassRestrictionsOnProperty(uri,
					((MergedRestriction) other).getOnProperty());
			return r != null && ((MergedRestriction) other).isDisjointWith(r, context, ttl, log);
		}
