...
public class GenericOntology extends Ontology {
	...
	public void create() {
		...
		while (it.hasNext()) {
				...
				if (o instanceof Resource) {
					...
					if (TypeExpression.OWL_CLASS.equals(classURI)) {
						...
						if (!ontClassSetup.containsKey(subj.getURI())) {
							...
							// + subj.getURI());
							OntClassInfoSetup setup = createNewOntClassInfo(subj.getURI(), fac, ++idx);
						...
						if (!rdfClassSetup.containsKey(subj.getURI())) {
							...
							// + subj.getURI());
							RDFClassInfoSetup setup = createNewRDFClassInfo(subj.getURI(), fac, ++idx);
							rdfClassSetup.put(subj.getURI(), setup);
							tmpSetup = setup;
						} else {
							tmpSetup = rdfClassSetup.get(subj.getURI());
						}
					} else if (DatatypeProperty.MY_URI.equals(classURI) || ObjectProperty.MY_URI.equals(classURI)) {
						props.add(subj);
					}
					if (tmpSetup != null) {
			...
			} else if (TypeExpression.PROP_RDFS_SUB_CLASS_OF.equals(el.getPredicate())) {
		...
		for (Resource p : props) {
			Resource domain = (Resource) p.getProperty(Property.PROP_RDFS_DOMAIN);
			if (domain != null) {
				...
				if (Arrays.asList(p.getTypes()).contains(DatatypeProperty.MY_URI)) {
					tmpSetup = setup.addDatatypeProperty(p.getURI());
				} else if (Arrays.asList(p.getTypes()).contains(ObjectProperty.MY_URI)) {
					tmpSetup = setup.addObjectProperty(p.getURI());
				} else {
					LogUtils.logWarn(SharedResources.moduleContext, GenericOntology.class, "create",
							"Found property that is neither object property nor datatype property: " + p.getURI());
				}
				if (tmpSetup != null) {
					propSetup.put(p.getURI(), tmpSetup);
					tmpSetup.setProperty(Resource.PROP_RDF_TYPE, p.getProperty(Resource.PROP_RDF_TYPE));
				}
			} else {
				LogUtils.logWarn(SharedResources.moduleContext, GenericOntology.class, "create",
		...
		for (Entry<Resource, List<Resource>> entry : defSubClass.entrySet()) {
			...
			if (setup == null) {
				LogUtils.logWarn(SharedResources.moduleContext, GenericOntology.class, "create",
						"No setup available for class " + r.getURI() + " to setup super classes.");
				continue;
			}
			...
			for (Object obj : lst) {
				if (!(obj instanceof Resource)) {
					LogUtils.logWarn(SharedResources.moduleContext, GenericOntology.class, "create",
							"The class " + r.getURI() + " has a super class definition that is not a resource.");
					continue;
				}
				...
				if (isRestriction) {
					...
					String onprop = ((Resource) (r.getProperty(PropertyRestriction.PROP_OWL_ON_PROPERTY))).getURI();
		...
		while (rit.hasNext()) {
			...
			for (String s : lst) {
				...
				if (setup != null) {
					...
					// ManagedIndividual
					GenericManagedIndividual m = new GenericManagedIndividual(r.getType(), r.getURI());
			...
			if (o instanceof List) {
				...
				for (Object el : lst) {
					if (!(el instanceof Resource))
						continue;
					ManagedIndividual m = individuals.get(((Resource) el).getURI());
					col.add(m);
				}
				...
				if (setup != null) {
					ManagedIndividual[] arr = (ManagedIndividual[]) col.toArray(new ManagedIndividual[] {});
