		...
		for (Enumeration e = r1.getPropertyURIs(); e.hasMoreElements();) {
			...
			if (val instanceof Resource) {
				...
				while (li.hasNext()) {
					...
					if (el instanceof Resource) {
						Resource specval = specialized.get(idxdummy.set(((Resource) el)));
						list.add(specval);
					} else {
						list.add(el);
					}
			...
			if (!r2.setProperty(propURI, val))
				if (!Resource.PROP_RDF_TYPE.equals(propURI)) {
					...
					// rdf:type
					LogUtils.logDebug(
							SharedResources.moduleContext, Specializer.class, "copyProperties", new Object[] {
									"Property ", propURI, " could not be correctly set for resource ", r1.getURI() },
							null);
				}
		}
	}
	...
	private Resource specializeSingle(Resource r) {
		...
		if (types == null || types.length == 0) {
			...
			if (type == null) {
				...
				substitution = OntologyManagement.getInstance().getResource(type, r.getURI());
			}
