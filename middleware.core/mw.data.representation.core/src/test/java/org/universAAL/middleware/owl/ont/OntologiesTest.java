...
public class OntologiesTest extends TestCase {
	...
	protected void logInfo(Object args) {
		...
		LogUtils.logInfo(SharedResources.moduleContext, getClass(), callingMethod.getMethodName(),
				new Object[] { args }, null);
	}

	public void testTestOnt() {
		logInfo("- unit test - ");
		Ontology o = OntologyManagement.getInstance().getOntology(ont.getInfo().getURI());
		assertFalse(o == null);
		// assertTrue(o == ont);

		Set subs = OntologyManagement.getInstance().getNamedSubClasses(ManagedIndividual.MY_URI, true, false);
	...
	public void testSpecialization() {
		...
		// try again
		assertTrue(MyResource.MY_URI.equals(OntologyManagement.getInstance().getMostSpecializedClass(
				new String[] { MyResource.MY_URI, "somethingcompletelyDifferent", "somethingNotSoDifferent" })));
		assertTrue(MyResource.MY_URI.equals(OntologyManagement.getInstance().getMostSpecializedClass(
				new String[] { "somethingcompletelyDifferent", MyResource.MY_URI, "somethingNotSoDifferent" })));
		assertTrue(MyResource.MY_URI.equals(OntologyManagement.getInstance().getMostSpecializedClass(
				new String[] { "somethingcompletelyDifferent", "somethingNotSoDifferent", MyResource.MY_URI })));

		// trying with ManagedIndividuals
		assertTrue(MyClass1Sub1.MY_URI.equals(OntologyManagement.getInstance().getMostSpecializedClass(
				new String[] { MyClass1.MY_URI, MyClass1Sub1.MY_URI, "somethingNotSoDifferent" })));
	}
}
