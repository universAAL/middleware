...
public class GraphIteratorTest extends TestCase {
	...
	class Triple {
		...
		public String toString() {
			return o[0].toString() + "\t" + o[1].toString() + "\t" + o[2].toString();
		}
	}
	...
	public void test1() {
		...
		while (it.hasNext()) {
			...
			assertTrue(el != null);
			assertTrue("root_resource".equals(((Resource) el.getSubject()).getURI()));
	...
	private void verify() {
		...
		while (it.hasNext()) {
			GraphIteratorElement el = (GraphIteratorElement) it.next();
			assertTrue(el != null);
			Triple t = new Triple(el.getSubject(), el.getPredicate(), el.getObject());
