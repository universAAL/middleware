...
public class LengthRestrictionTest extends TestCase {
	...
	private void testPatternHasMember(String pattern, String member, boolean shouldMatch) {
		if ((!member.matches(pattern) && shouldMatch) || (member.matches(pattern) && !shouldMatch)) {
			System.out.println("Pattern does not match -> Restrictions cannot match");
			assertTrue(false);
		}
