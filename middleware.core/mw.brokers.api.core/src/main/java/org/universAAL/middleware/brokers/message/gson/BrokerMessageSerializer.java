...
public class BrokerMessageSerializer implements JsonSerializer<BrokerMessage>, JsonDeserializer<BrokerMessage> {
	...
	public BrokerMessage deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
		...
		if (ele instanceof JsonPrimitive) {
			...
			switch (myEyum) {
			case DeployMessage:
				message = GsonParserBuilder.getInstance().fromJson(json, DeployMessage.class);
				break;
			case ControlMessage:
				message = GsonParserBuilder.getInstance().fromJson(json, ControlMessage.class);
				break;
			default:
				break;
			}
