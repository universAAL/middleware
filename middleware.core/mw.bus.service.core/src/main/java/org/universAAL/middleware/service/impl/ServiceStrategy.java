...
public class ServiceStrategy extends BusStrategy {
	...
	public ServiceStrategy(CommunicationModule commModule) {
		...
		LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "ServiceStrategy",
				new Object[] { "This instance is ", isCoordinator ? "" : "NOT ", "the coordinator." }, null);
	 ...
	 */
	void addRegParams(String calleeID, ServiceProfile[] realizedServices, boolean throwOnDuplicateReg) {
		if (realizedServices == null || calleeID == null || !(getBusMember(calleeID) instanceof ServiceCallee))
			return;
		...
		synchronized (localServicesIndex) {
			...
			for (int i = 0; i < realizedServices.length; i++) {
				...
				if (realizedServices[i] == null || realizedServices[i].getTheService() == null)
				...
				// provider
				ServiceRealization registration = new ServiceRealization(calleeID, realizedServices[i]);
			...
			for (Data dat : tmp) {
				...
				localServicesIndex.addServiceRealization(dat.processURI, dat.registration);
	 ...
	 *            services.
	 */
	private void callServices(BusMessage m, Vector<HashMap<String, Object>> matches) {
	 ...
	 *            the call context
	 */
	private void callService(BusMessage call, PeerCard receiver, HashMap<String, Object> match) {
		...
		try {
			...
			if (call.getSender() == null) {
				LogUtils.logError(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "callServices",
						new Object[] { "Call.getSender() is null - ignoring." }, null);
			} else if (call.getSender().getPeerID() == null) {
				LogUtils.logError(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "callServices",
						new Object[] { "Call.getSender().getPeerID() is null - ignoring." }, null);
			}
			...
			if (receiver == null) {
				...
				LogUtils.logError(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "callServices",
						new Object[] { "Receiver.getPeerID() is null - ignoring." }, null);
			}
	 ...
	 */
	private void callStartDialog(ArrayList<ServiceRealization> matchingServices, String vendor, BusMessage m) {
		...
		for (ServiceRealization sr : matchingServices) {
			...
			if (vendor.equals(String.valueOf(s.getProperty(InitialServiceDialog.PROP_HAS_VENDOR)))) {
				...
				if (processURI instanceof String && calleeID instanceof String) {
					...
					((ServiceBusImpl) bus).assessContentSerialization(sc);
					BusMessage call = new BusMessage(MessageType.p2p_request, sc, bus);
					PeerCard receiver = AbstractBus.getPeerFromBusResourceURI((String) calleeID);
					call.setReceiver(receiver);
	...
	private void sendServiceResponse(BusMessage m) {
		...
		synchronized (matches) {
			...
			if (size == numTimedOut) {
				...
				boolean[] nmsf = new boolean[size], rto = new boolean[size], ssf = new boolean[size],
						den = new boolean[size];
				// responses with CallStatus.SUCCEEDED
				ArrayList<HashMap<String, Object>> goods = new ArrayList<HashMap<String, Object>>(size);
				...
				case 0:
					if (bads == 0) {
						...
						ServiceResponse sr = (ServiceResponse) bad.get(CONTEXT_RESPONSE_MESSAGE);
						if (sr == null)
				...
				default:
					size = goods.size();
					List<AggregatingFilter> aggregations = ((ServiceRequest) m.getContent()).getOutputAggregations();
					...
					if (size == 1) {
						...
						for (HashMap<String, Object> ctxt : goods) {
							tmp = (ServiceResponse) ctxt.get(CONTEXT_RESPONSE_MESSAGE);
		...
		((ServiceBusImpl) bus).assessContentSerialization((Resource) m.getContent());
		if (m.receiverResidesOnDifferentPeer()) {
			send(m);
		} else {
	 ...
	 */
	private void prepareRequestedOutput(ServiceResponse sr, Map<String, Object> ctxt) {
		if (sr instanceof MultiServiceResponse) {
			List<ServiceResponse> responses = ((MultiServiceResponse) sr).getResponses();
	 ...
	 *            HashMap of bindings for the ProcessOutputs
	 */
	private void prepareRequestedOutput(List<ProcessOutput> outputs, Map<String, Object> context) {
		if (outputs != null && !outputs.isEmpty())
			for (int i = outputs.size() - 1; i > -1; i--) {
				...
				if (binding != null) {
					...
					// side).
					ServiceResponse sr = (ServiceResponse) context.get(CONTEXT_RESPONSE_MESSAGE);
	...
	private List<String> getNonAbstractSuperClasses(Service s) {
		...
		if (info != null) {
			if (OntologyManagement.getInstance().isRegisteredClass(s.getClassURI(), false))
				lst.add(classURI);
		}
	 ...
	 * @return the output
	 */
	private Object getOutputValue(Map<String, Object> context, AggregatingFilter af) {
		List<ProcessOutput> outputs = ((ServiceResponse) context.get(CONTEXT_RESPONSE_MESSAGE)).getOutputs();
		if (outputs == null || outputs.isEmpty())
			return null;
		...
		for (String key : context.keySet()) {
			...
			if (o instanceof Resource) {
				...
				if (o instanceof AggregatingFilter && ((AggregatingFilter) o).getTheFunction() == af.getTheFunction()
	 ...
	 */
	private <V> Vector<V> getVector(Hashtable<String, Vector<V>> table, String key) {
	...
	private void logTrace(String methodName, Object[] obj) {
		try {
			...
			if (id != null)
				LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, methodName, obj, null);
		} catch (Exception e) {
			LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "logTrace",
	...
	public void handle(BusMessage msg, String senderID) {
			...
			if (res.getType().equals(TYPE_uAAL_SERVICE_BUS_SUBSCRIPTION) && isCoordinator) {
				if (RES_STATUS_DEREGISTERED.equals(res.getProperty(PROP_uAAL_REGISTRATION_STATUS))) {
					...
					synchronized (allServicesIndex) {
						...
						if (arrAS != null) {
							for (Iterator<AvailabilitySubscription> i = arrAS.iterator(); i.hasNext();) {
		...
		case MessageType.P2P_REPLY:
			if (res instanceof ServiceResponse) {
				HashMap<String, Object> callContext = allWaitingCalls.remove(msg.getInReplyTo());
				...
				if (isCoordinator) {
					...
					BusMessage request = (BusMessage) callContext.get(CONTEXT_REQUEST_MESSAGE);
					...
					if (allCalls == null) {
						...
						LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "handle",
		...
		case MessageType.P2P_REQUEST:
			if (res instanceof ServiceCall) {
				...
				synchronized (localServicesIndex) {
					sr = localServicesIndex.getServiceRealizationByID(((ServiceCall) res).getProcessURI());
				}
				if (sr != null) {
		...
		case MessageType.REQUEST:
			...
			if (isCoordinator) {
				...
				if (request.getRequestedService() instanceof InitialServiceDialog) {
					...
					Object csc = s.getInstanceLevelFixedValueOnProp(InitialServiceDialog.PROP_CORRELATED_SERVICE_CLASS);
					if (csc instanceof Resource) {
						Object hv = s.getInstanceLevelFixedValueOnProp(InitialServiceDialog.PROP_HAS_VENDOR);
						if (request.getURI().startsWith(InitialServiceDialog.SERVICE_REQUEST_URI_PREFIX_INFO)) {
							synchronized (startDialogs) {
								ArrayList<ServiceRealization> v = startDialogs.get(csc.toString());
							...
							synchronized (startDialogs) {
								callStartDialog(startDialogs.get(csc.toString()), hv.toString(), msg);
							}
						} else
				...
				synchronized (allServicesIndex) {
					LogUtils.logTrace(
							ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "handle", new Object[] {
									ServiceBus.LOG_MATCHING_START, new UnmodifiableResource(request), " ", logID },
							null);

					ArrayList<ServiceRealization> arrServices = allServicesIndex.get(serviceURI);
					if (arrServices == null) {
						logTrace("handle",
								new Object[] { ServiceBus.LOG_MATCHING_END, " No service available. ",
										ServiceBus.LOG_MATCHING_MISMATCH_CODE, Integer.valueOf(1030),
										ServiceBus.LOG_MATCHING_MISMATCH_DETAILS,
										" No service has registered for the requested serviceURI.", logID });
						...
						for (ServiceRealization sr : arrServices) {
							...
							logTrace("handle", new Object[] { ServiceBus.LOG_MATCHING_PROFILE, profileService.getType(),
									profileServiceURI, profileProviderURI, logID });
							HashMap<String, Object> context = matches(caller, request, sr, logID);
							if (context != null) {
								matches.add(context);
								logTrace("handle", new Object[] { ServiceBus.LOG_MATCHING_SUCCESS, logID });
							} else
								logTrace("handle", new Object[] { ServiceBus.LOG_MATCHING_NOSUCCESS, logID });
						}
					}
				}
				int matchesFound = 0;
				...
				if (logID != null) {
					...
					logTrace("handle", new Object[] { ServiceBus.LOG_MATCHING_PROFILES_END, " Found ",
				...
				if (size == 0) {
					sendNoMatchingFound(msg);
					logTrace("handle", new Object[] { ServiceBus.LOG_MATCHING_END, "found ",
					...
					if (size > 1) {
						...
						if (filters != null && filters.size() > 0) {
							...
							for (AggregatingFilter af : filters) {
								List<?> params = af.getFunctionParams();
								String[] pp = null;
								if (params != null && !params.isEmpty() && params.get(0) instanceof PropertyPath)
									pp = ((PropertyPath) params.get(0)).getThePath();
								if (pp == null || pp.length != 2 || !Service.PROP_OWLS_PRESENTS.equals(pp[0]))
									continue;
								switch (af.getTheFunction().ord()) {
								case AggregationFunction.ONE_OF:
									break;
				...
				LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "handle",
						new Object[] {
								"Received a message of type 'request'. A peer has send this message and thought I would be the coordinator, but I'm not. Ignoring it." },
						null);
			} else if (isCoordinatorKnown()) {
				localWaitingCallers.addLocalWaitier(msg.getID(), senderID);
	...
	private boolean handleResponseOfInjectedCall(BusMessage msg, HashMap<String, Object> callContext) {
		if (callContext == null)
			return false;
	 ...
	 * @param matchingServices
	 */
	private void replyToInitialDialogInfoRequest(BusMessage m, ArrayList<ServiceRealization> matchingServices) {
		if (matchingServices == null) {
			sendNoMatchingFound(m);
			return;
		}
	...
	private void replyToLocalCaller(BusMessage msg) {
		...
		if (replyOf == null) {
			...
			// => ignore!
			LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "replyToLocalCaller",
					new Object[] { "Message of type REPLY, but not containing inReplyTo. Ignoring it." }, null);
		} else {
			...
			if (callerID == null) {
				...
				// => ignore!
				LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "replyToLocalCaller",
						new Object[] { "There is no caller. To whom should I then reply? Ignoring it." }, null);
			} else {
				...
				if (caller instanceof ServiceCaller)
					...
					// => ignore!
					LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "replyToLocalCaller",
							new Object[] { "The caller '" + callerID + "' is no ServiceCaller. Ignoring it." }, null);
				}
			}
		}
	}
	...
	private void indexServices(ServiceProfile prof, ServiceRealization registration, String processURI) {
		...
		if (theService instanceof InitialServiceDialog) {
			...
			if (!(correlService instanceof Resource)) {
				LogUtils.logWarn(ServiceBusImpl.getModuleContext(), ServiceStrategy.class, "indexServices",
			...
			synchronized (allServicesIndex) {
				for (String serviceURI : serviceURIs) {
					ArrayList<ServiceRealization> arrsr = allServicesIndex.get(serviceURI);
					if (arrsr == null) {
	...
	private void notifyLocalSubscriber(String caller, String request, String realization, boolean registers) {
		Vector<AvailabilitySubscription> v = localSubscriptionsIndex.get(caller);
		if (v != null) {
	...
	void removeAvailabilitySubscription(String callerID, AvailabilitySubscriber subscriber, String requestURI) {
		if (requestURI == null || subscriber == null || localSubscriptionsIndex.get(callerID) == null)
			return;

		Vector<AvailabilitySubscription> v = localSubscriptionsIndex.get(callerID);
		String serviceClassURI = null;
		if (v != null) {
			for (Iterator<AvailabilitySubscription> i = v.iterator(); i.hasNext();) {
				AvailabilitySubscription as = i.next();
		...
		if (isCoordinator) {
			synchronized (allServicesIndex) {
				ArrayList<AvailabilitySubscription> arrAS = allSubscriptionsIndex.get(serviceClassURI);
				if (arrAS != null)
					for (Iterator<AvailabilitySubscription> i = arrAS.iterator(); i.hasNext();) {
						AvailabilitySubscription as = i.next();
	...
	void removeRegParams(String calleeID) {
		if (calleeID == null || !(getBusMember(calleeID) instanceof ServiceCallee))
			return;
	...
	public ServiceProfile[] getAllServiceProfiles(String serviceURI) {
		...
		synchronized (this) {
			while (!this.localServiceSearchResults.exist(serviceURI) && maxRetry > retryCount) {
				try {
					wait(msTimeout);
					retryCount++;
	...
	public HashMap<String, List<ServiceProfile>> getAllServiceProfilesWithCalleeIDs(String serviceURI) {
		return getCoordinatorServicesWithCalleeIDs(serviceURI);
	}

	private HashMap<String, List<ServiceProfile>> getCoordinatorServicesWithCalleeIDs(String serviceURI) {
		HashMap<String, List<ServiceProfile>> map = new HashMap<String, List<ServiceProfile>>();
		if (this.isCoordinator) {
			synchronized (allServicesIndex) {
				ArrayList<ServiceRealization> neededProfiles = allServicesIndex.get(serviceURI);
