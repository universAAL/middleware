...
public final class ServiceBusImpl extends AbstractBus implements ServiceBus {
	...
	public static class CallInjectorImpl implements CallInjector {
		...
		public void brokerCall(String callerID, String receiver, BusMessage call) {
			brokerCall(callerID, AbstractBus.getPeerFromBusResourceURI(receiver), call);
		}
	}
	...
	public synchronized void assessContentSerialization(Resource content) {
		if (Constants.debugMode()) {
			LogUtils.logDebug(context, ServiceBusImpl.class, "assessContentSerialization",
	...
	public static synchronized void startModule(ModuleContext mc, Object[] serviceBusShareParams,
			Object[] serviceBusFetchParams, Object[] serviceBusInjectShareParams,
		...
		if (theServiceBus == null) {
			...
			theCallInjector = new CallInjectorImpl((ServiceStrategy) theServiceBus.busStrategy);
	 ...
	 */
	public void addAvailabilitySubscription(String callerID, AvailabilitySubscriber subscriber,
			ServiceRequest request) {
		if (callerID != null && registry.getBusMemberByID(callerID) instanceof ServiceCaller) {
	...
	public void addNewServiceProfiles(String calleeID, ServiceProfile[] realizedServices, boolean throwOnDuplicateReg) {
		if (calleeID != null) {
			((ServiceStrategy) busStrategy).addRegParams(calleeID, realizedServices, throwOnDuplicateReg);
	...
	public void removeMatchingProfiles(String calleeID, ServiceProfile[] realizedServices) {
		if (calleeID != null) {
			((ServiceStrategy) busStrategy).removeMatchingRegParams(calleeID, realizedServices);
