...
public class UserInterfaceService extends Service {
	 ...
	 */
	public static final ServiceProfile createServiceProfile(String serviceClassURI, String vendor, String description,
			String startServiceURI) {
		return createServiceProfile(new UserInterfaceService(startServiceURI), serviceClassURI, vendor, description,
				startServiceURI);
	}
	...
	protected static ServiceProfile createServiceProfile(UserInterfaceService uis, String serviceClassURI,
			String vendor, String description, String startServiceURI) {
		uis.setProperty(PROP_CORRELATED_SERVICE_CLASS, new Resource(serviceClassURI, true));
	 ...
	 * @return The description of the service profile.
	 */
	protected static String getUIServiceDescription(UserInterfaceService requestedService, String serviceClassURI,
			String vendor, ServiceCaller theCaller) {
		requestedService.addInstanceLevelRestriction(MergedRestriction
				.getFixedValueRestriction(PROP_CORRELATED_SERVICE_CLASS, new Resource(serviceClassURI, true)),
		...
		req.addSimpleOutputBinding(new ProcessOutput(OUTPUT_INSTANCE_INFO), new String[] { PROP_DESCRIPTION });
	...
	protected static UserInterfaceService[] getUIServiceInfo(UserInterfaceService requestedService,
		...
		requestedService.addInstanceLevelRestriction(MergedRestriction
				...
				new String[] { PROP_CORRELATED_SERVICE_CLASS });
		ServiceRequest req = new ServiceRequest(SERVICE_REQUEST_URI_PREFIX_INFO, 5, requestedService, null);
		req.addSimpleOutputBinding(new ProcessOutput(OUTPUT_INSTANCE_INFO), new String[] { MY_URI });
		ServiceResponse resp = theCaller.call(req);
		if (resp.getCallStatus() == CallStatus.succeeded) {
			List outputs = resp.getOutputs();
			if (outputs != null)
						...
						if (o instanceof List) {
							try {
								return (UserInterfaceService[]) ((List) o)
										.toArray(new UserInterfaceService[((List) o).size()]);
							} catch (Exception e) {
							}
						}
					}
	 ...
	 */
	public static final ServiceRequest getUIServiceRequest(String serviceClassURI, String vendor,
	 ...
	 * @return the UI service request.
	 */
	protected static ServiceRequest getUIServiceRequest(UserInterfaceService requestedService, String serviceClassURI,
