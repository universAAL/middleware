...
public class ProcessResult extends FinalizedResource {
	...
	public static final String PROP_OWLS_RESULT_WITH_OUTPUT = ProcessOutput.OWLS_PROCESS_NAMESPACE + "withOutput";
	public static final String MY_URI = ProcessOutput.OWLS_PROCESS_NAMESPACE + "Result";
	...
	public static boolean checkEffects(Resource[] req, Resource[] offer, HashMap context, Long logID) {
		if (req == null || req.length == 0) {
			if (offer == null || offer.length == 0)
				...
				if (logID != null)
					LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ProcessResult.class, "checkEffects",
							new Object[] { ServiceBus.LOG_MATCHING_MISMATCH, "offered effect not requested",
									"\nnumber of offered effects: ", Integer.valueOf(offer.length),
									ServiceBus.LOG_MATCHING_MISMATCH_CODE, Integer.valueOf(1010),
		...
		if (offer == null || offer.length != req.length) {
			if (logID != null) {
				if (offer == null)
					LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ProcessResult.class, "checkEffects",
							new Object[] { ServiceBus.LOG_MATCHING_MISMATCH, "requested effect not offered",
									"\nnumber of requested effects: ", Integer.valueOf(req.length),
									ServiceBus.LOG_MATCHING_MISMATCH_CODE, Integer.valueOf(1011),
									ServiceBus.LOG_MATCHING_MISMATCH_DETAILS,
					...
					LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ProcessResult.class, "checkEffects",
							new Object[] { ServiceBus.LOG_MATCHING_MISMATCH, "number of effects do not match",
									"\nnumber of requested effects: ", Integer.valueOf(req.length),
									"\nnumber of offered effects: ", Integer.valueOf(offer.length),
									ServiceBus.LOG_MATCHING_MISMATCH_CODE, Integer.valueOf(1012),
									ServiceBus.LOG_MATCHING_MISMATCH_DETAILS,
		...
		for (int i = 0; i < req.length; i++) {
			if (!ProcessEffect.findMatchingEffect(req[i], offer, context)) {
				if (logID != null)
					LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ProcessResult.class, "checkEffects",
							new Object[] { ServiceBus.LOG_MATCHING_MISMATCH, "requested effect not offered",
									"\nrequested effect: ", req[i], ServiceBus.LOG_MATCHING_MISMATCH_CODE,
									Integer.valueOf(1013), ServiceBus.LOG_MATCHING_MISMATCH_DETAILS,
									"At least one of the requested effects is not offered by the service.", logID },
							null);
				return false;
			}
		}
	...
	public static boolean checkOutputBindings(Resource[] req, Resource[] offer, HashMap context, Long logID) {
		...
		if (offer == null || offer.length == 0) {
			if (logID != null)
				LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ProcessResult.class, "checkOutputBindings",
						new Object[] { ServiceBus.LOG_MATCHING_MISMATCH, "requested output not available",
								"\nnumber of requested outputs: ", Integer.valueOf(req.length),
								ServiceBus.LOG_MATCHING_MISMATCH_CODE, Integer.valueOf(1000),
								ServiceBus.LOG_MATCHING_MISMATCH_DETAILS,
				...
				if (logID != null) {
					LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ProcessResult.class, "checkOutputBindings",
							new Object[] { ServiceBus.LOG_MATCHING_MISMATCH, "requested output not available",
									...
									((ProcessOutput) req[i].getProperty(OutputBinding.PROP_OWLS_BINDING_TO_PARAM))
											.getURI(),
									"\nindex of requested output: ", Integer.valueOf(i),
									ServiceBus.LOG_MATCHING_MISMATCH_CODE, Integer.valueOf(1001),
									ServiceBus.LOG_MATCHING_MISMATCH_DETAILS,
	...
	public static ProcessResult toResult(Resource pr) {
		...
		if ((bindings == null && effects == null) || (bindings != null && !(bindings instanceof List))
	 ...
	 */
	public void addClassConversionOutputBinding(ProcessOutput toParam, PropertyPath sourceProp, TypeURI targetClass) {
	 ...
	 */
	public void addLangConversionOutputBinding(ProcessOutput toParam, PropertyPath sourceProp, String targetLang) {
	 ...
	 */
	public void addUnitConversionOutputBinding(ProcessOutput toParam, PropertyPath sourceProp, String targetUnit) {
	...
	public boolean isWellFormed() {
		return props.containsKey(PROP_OWLS_RESULT_WITH_OUTPUT) || props.containsKey(PROP_OWLS_RESULT_HAS_EFFECT);
	}
}
