...
public class AggregatingFilter extends FinalizedResource {
	...
	public static final String PROP_AGGREGATION_FUNCTION = VOCABULARY_NAMESPACE + "theFunction";
	public static final String PROP_AGGREGATION_PARAMS = VOCABULARY_NAMESPACE + "aggregationParameters";
	...
	private static boolean checkIntegrity(AggregationFunction func, List params) {
		...
		for (int i = 0; i < params.size(); i++) {
			...
			if ((PropertyPath.TYPE_PROPERTY_PATH.equals(func.getParameterType(i)) && !(o instanceof PropertyPath)))
				return false;
	...
	public AggregatingFilter(AggregationFunction func, List params, boolean asLiteral) {
		...
		if (func != null && params != null && checkIntegrity(func, params)) {
			props.put(PROP_AGGREGATION_FUNCTION, func);
			props.put(PROP_AGGREGATION_PARAMS, params);
		} else
	...
	public List getFunctionParams() {
		return (List) props.get(PROP_AGGREGATION_PARAMS);
	}
	...
	public AggregationFunction getTheFunction() {
		return (AggregationFunction) props.get(PROP_AGGREGATION_FUNCTION);
	}

	/**
	 * Returns true, if the PROP_AGGREGATION_FUNCTION and
	 * PROP_AGGREGATION_PARAMS contain the required key.
	 */
	public boolean isWellFormed() {
		return props.containsKey(PROP_AGGREGATION_FUNCTION) && props.containsKey(PROP_AGGREGATION_PARAMS);
	}
	...
	public boolean setProperty(String propURI, Object value) {
		...
		List params = null;
		if (propURI.equals(PROP_AGGREGATION_FUNCTION)) {
		...
		} else if (propURI.equals(PROP_AGGREGATION_PARAMS) && value instanceof List && !((List) value).isEmpty()) {
			func = getTheFunction();
			params = (List) value;
		} else
			return false;
	...
	public AggregatingFilter toLiteral() {
		...
		result.props.put(PROP_AGGREGATION_FUNCTION, getProperty(PROP_AGGREGATION_FUNCTION));
		result.props.put(PROP_AGGREGATION_PARAMS, getProperty(PROP_AGGREGATION_PARAMS));
		return result;
	}
}
