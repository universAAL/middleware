...
public final class MultiServiceResponse extends ServiceResponse {
	...
	public void addOutput(ProcessOutput output) {
		throw new UnsupportedOperationException("Can not add an output to a MultiServiceResponse.");
	}
	...
	public void addResponse(ServiceResponse response) {
		...
		if (response instanceof MultiServiceResponse) {
			List<ServiceResponse> l = ((MultiServiceResponse) response).getResponses();
			for (ServiceResponse s : l)
				addResponse(s);
			return;
		}
		List<ServiceResponse> l = (List<ServiceResponse>) props.get(PROP_SERVICE_HAS_OUTPUT);
		if (l == null)
