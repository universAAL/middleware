...
public class OutputBinding {
	...
	public static final String PROP_OWLS_BINDING_VALUE_FORM = ProcessOutput.OWLS_PROCESS_NAMESPACE + "valueForm";
	public static final String PROP_OWLS_BINDING_VALUE_FUNCTION = ProcessOutput.OWLS_PROCESS_NAMESPACE
			+ "valueFunction";
	public static final String TYPE_OWLS_OUTPUT_BINDING = ProcessOutput.OWLS_PROCESS_NAMESPACE + "OutputBinding";
		...
		if (o instanceof Resource && ((Resource) o).isAnon()) {
			...
			for (Enumeration e = ((Resource) o).getPropertyURIs(); e.hasMoreElements();) {
				...
				if (Resource.PROP_RDF_TYPE.equals(key)) {
					...
					if (!(key instanceof AggregatingFilter) && !Conversion.checkConversion(key))
						return false;
				} else
					return false;
			}
	...
	static Resource constructClassConversionBinding(ProcessOutput toParam, PropertyPath sourceProp,
			TypeURI targetClass) {
		Resource result = new Resource();
		result.addType(TYPE_OWLS_OUTPUT_BINDING, true);
		result.setProperty(PROP_OWLS_BINDING_TO_PARAM, toParam);
	...
	static Resource constructLanguageConversionBinding(ProcessOutput toParam, PropertyPath sourceProp,
			String targetLang) {
		Resource result = new Resource();
		result.addType(TYPE_OWLS_OUTPUT_BINDING, true);
		result.setProperty(PROP_OWLS_BINDING_TO_PARAM, toParam);
	...
	static Resource constructUnitConversionBinding(ProcessOutput toParam, PropertyPath sourceProp, String targetUnit) {
		Resource result = new Resource();
		result.addType(TYPE_OWLS_OUTPUT_BINDING, true);
		result.setProperty(PROP_OWLS_BINDING_TO_PARAM, toParam);
		result.setProperty(PROP_OWLS_BINDING_VALUE_FUNCTION,
	...
	static boolean findMatchingBinding(Resource req, Resource[] offer, HashMap context) {
		...
		for (int i = 0; i < offer.length; i++) {
			...
			if (offeredValue != null && offeredValue.equals(srcPath)) {
				...
				ProcessOutput toParam = (ProcessOutput) offer[i].getProperty(PROP_OWLS_BINDING_TO_PARAM);
