...
public class ServiceCall extends FinalizedResource implements UtilityCall {
     ...
     */
    public static final String MY_URI = ProcessInput.OWLS_PROCESS_NAMESPACE
	    + "Perform";
     ...
     */
    public static final String PROP_OWLS_BINDING_VALUE_DATA = ProcessInput.OWLS_PROCESS_NAMESPACE
	    + "valueData";

    /**
     * A property key that corresponds to the URI of the OWL-S perform process.
     */
    public static final String PROP_OWLS_PERFORM_PROCESS = ProcessInput.OWLS_PROCESS_NAMESPACE
	    + "process";
     ...
     * input resources.
     */
    public static final String PROP_OWLS_PERFORM_HAS_DATA_FROM = ProcessInput.OWLS_PROCESS_NAMESPACE
	    + "hasDataFrom";
	...
	if (inputURI != null && value != null) {
	    ...
	    binding.setProperty(OutputBinding.PROP_OWLS_BINDING_TO_PARAM,
		    new ProcessInput(inputURI));
		...
		if (o instanceof List && !((List) o).isEmpty()) {
			...
			if (binding instanceof Resource
				&& TYPE_OWLS_INPUT_BINDING
					.equals(((Resource) binding).getType())) {
			    Object toParam = ((Resource) binding)
				    .getProperty(OutputBinding.PROP_OWLS_BINDING_TO_PARAM);
			    if (toParam instanceof Resource
				    && !((Resource) toParam).isAnon()
				    && ProcessInput.MY_URI
					    .equals(((Resource) toParam)
						    .getType())) {
				if (!addInput(
					toParam.toString(),
					((Resource) binding)
