...
public class ServiceMatcher {
	...
	private boolean matchService(ServiceWrapper superset, ServiceWrapper subset) {
		...
		if (!ManagedIndividual.checkMembership(superService.getClassURI(), subsetService))
			return false;
		/*
	...
	private boolean matchInitialServiceDialog(ServiceWrapper superset, ServiceWrapper subset) {
		...
		if (superService instanceof InitialServiceDialog) {
			...
			if (!(superClass instanceof Resource) || !(subClass instanceof Resource))
				return false;

			if (!(superClass.toString().equals(subClass.toString()))) {
				OntClassInfo subOntClassInfo = OntologyManagement.getInstance().getOntClassInfo(subClass.toString());
			...
			// 2. check vendor
			Object superVendor = superset.getInitialServiceDialogProperty(InitialServiceDialog.PROP_HAS_VENDOR);
			Object subVendor = subset.getInitialServiceDialogProperty(InitialServiceDialog.PROP_HAS_VENDOR);
	...
	private int matchRestrictions(ServiceWrapper superset, ServiceWrapper subset, HashMap context, Long logID) {
		...
		if (restrProps != null && restrProps.length > 0) {
			for (int i = 0; i < restrProps.length; i++) {
				...
				TypeExpression supersetInsRestr = supersetService.getInstanceLevelRestrictionOnProp(restrProp);
				...
				if (Service.PROP_OWLS_PRESENTS.equals(restrProp)) {
					...
					if (supersetInsRestr instanceof PropertyRestriction) {
						restrProp = ((PropertyRestriction) supersetInsRestr).getOnProperty();
						if (restrProp == null)
							// strange!
							continue;
					...
					TypeExpression subsetClsRestr = Service.getClassRestrictionsOnProperty(subsetService.getClassURI(),
							restrProp);

					// offer instance level restrictions
					TypeExpression subsetInsRestr = subsetService.getInstanceLevelRestrictionOnProp(restrProp);

					// request class level restrictions
					TypeExpression supersetClsRestr = Service
							.getClassRestrictionsOnProperty(supersetService.getClassURI(), restrProp);
					...
					if (!supersetAllRestr.matches(subsetAllRestr, context, -1, null)) {
						if (logID != null)
							LogUtils.logTrace(ServiceBusImpl.getModuleContext(), ServiceRealization.class, "matches",
									new Object[] { ServiceBus.LOG_MATCHING_MISMATCH,
											"no subset relationship for restricted property", "\nrestricted property: ",
											restrProp, ServiceBus.LOG_MATCHING_MISMATCH_CODE, Integer.valueOf(1022),
											ServiceBus.LOG_MATCHING_MISMATCH_DETAILS,
	...
	private boolean matchEffects(ServiceWrapper superset, ServiceWrapper subset, HashMap context, Long logID) {
		// check effects
		if (!ProcessResult.checkEffects(superset.getEffects(), subset.getEffects(), context, logID))
			return false;

		return true;
	}

	private boolean matchOutputs(ServiceWrapper superset, ServiceWrapper subset, HashMap context, Long logID) {
		// check output bindings
		if (!ProcessResult.checkOutputBindings(superset.getOutputs(), subset.getOutputs(), context, logID))
			return false;
	...
	private void processNonSemanticInput(ServiceWrapper superset, HashMap context) {
		...
		try {
			...
			LogUtils.logDebug(ServiceBusImpl.getModuleContext(), ServiceRealization.class, "matches",
					new Object[] {
							"Exception occured when trying to get non-semantic parameters from AapiServiceRequest" },
					ex);
		}
		if (nonSemanticInput != null) {
			context.put(AapiServiceRequest.PROP_NON_SEMANTIC_INPUT, nonSemanticInput);
		}
	}
