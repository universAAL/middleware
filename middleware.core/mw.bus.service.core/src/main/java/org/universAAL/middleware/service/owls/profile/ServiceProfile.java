...
public class ServiceProfile extends FinalizedResource implements UtilityAdvertisement {
	...
	public ServiceProfile(Service s, String processURI) {
		...
		OntClassInfo oci = OntologyManagement.getInstance().getOntClassInfo(s.getClassURI());
	...
	public void addClassConversionOutputBinding(ProcessOutput toParam, String[] sourceProp, TypeURI targetClass) {
		if (toParam != null && sourceProp != null && targetClass != null) {
			theResult().addClassConversionOutputBinding(toParam, new PropertyPath(null, true, sourceProp), targetClass);
		}
	}
	...
	public void addLangConversionOutputBinding(ProcessOutput toParam, String[] sourceProp, String targetLang) {
		if (toParam != null && sourceProp != null && targetLang != null) {
			theResult().addLangConversionOutputBinding(toParam, new PropertyPath(null, true, sourceProp), targetLang);
		}
	}
	...
	public void addUnitConversionOutputBinding(ProcessOutput toParam, String[] sourceProp, String targetUnit) {
		if (toParam != null && sourceProp != null && targetUnit != null) {
			theResult().addUnitConversionOutputBinding(toParam, new PropertyPath(null, true, sourceProp), targetUnit);
		}
	}
	...
	public boolean isEmpty() {
		return props.get(PROP_OWLS_PROFILE_HAS_INPUT) == null && props.get(PROP_OWLS_PROFILE_HAS_OUTPUT) == null
	...
	public boolean setProperty(String propURI, Object value) {
		...
		if (propURI == null || value == null || !(value instanceof ProfileParameter) && props.containsKey(propURI)) {
			return false;
		}
		if (Service.PROP_OWLS_PRESENTED_BY.equals(propURI)) {
			...
			} else if (!(value instanceof Resource) || ((Resource) value).numberOfProperties() != 0) {
				return false;
			}
			if (!((Resource) value).isAnon() && Resource.isQualifiedName(((Resource) value).getURI())) {
				props.put(propURI, value);
				return true;
			}
			...
			props.put(propURI, value);
			return true;
		} else if (value instanceof ProfileParameter && ((ProfileParameter) value).isWellFormed()) {
			Class subclassOfProfileParameter = (Class) nonFunctionalParams.get(propURI);
			if (subclassOfProfileParameter != null && subclassOfProfileParameter.isAssignableFrom(value.getClass())) {
				props.put(propURI, value);
				return true;
			}
		}
