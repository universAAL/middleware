...
public class ServiceBusOntology extends Ontology {
	...
	public void create() {
		...
		createNewRDFClassInfo(ServiceCall.MY_URI, factory, 3).addInstance(ServiceCall.THIS_SERVICE_CALL);
		...
		oci.addDatatypeProperty(Service.PROP_NUMBER_OF_VALUE_RESTRICTIONS).setFunctional();
		...
		oci.setResourceComment(
				"The class of all services starting an initial dialog correlated to a specific service class");
		oci.setResourceLabel("User Interface Service");
		oci.addSuperClass(Service.MY_URI);
		oci.addObjectProperty(UserInterfaceService.PROP_CORRELATED_SERVICE_CLASS).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				UserInterfaceService.PROP_CORRELATED_SERVICE_CLASS, TypeMapper.getDatatypeURI(Resource.class), 1, 1));
		oci.addDatatypeProperty(UserInterfaceService.PROP_DESCRIPTION).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				UserInterfaceService.PROP_DESCRIPTION, TypeMapper.getDatatypeURI(String.class), 1, 1));
		oci.addObjectProperty(UserInterfaceService.PROP_HAS_INFO_RETRIEVAL_PROCESS).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				UserInterfaceService.PROP_HAS_INFO_RETRIEVAL_PROCESS, TypeMapper.getDatatypeURI(Resource.class), 1, 1));
		oci.addObjectProperty(UserInterfaceService.PROP_HAS_VENDOR).setFunctional();
		oci.addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
				UserInterfaceService.PROP_HAS_VENDOR, TypeMapper.getDatatypeURI(Resource.class), 1, 1));
