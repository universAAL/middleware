...
public class ProfileUtil {
	...
	static {
		...
		SERVICE_GET_LAMPS_INDOORS = LAMP_SERVER_NAMESPACE + "srv_getLampsIndoor";
		...
		ppLocationType = new String[] { DeviceService.PROP_CONTROLS, PhysicalThing.PROP_PHYSICAL_LOCATION };
	...
	public static ServiceProfile create_getControlledLamps(boolean dev) {
		...
		getControlledLamps.addOutput(OUTPUT_CONTROLLED_LAMPS, Lamp.MY_URI, 0, -1, ppControls);
	 ...
	 * @return
	 */
	public static ServiceProfile create_getControlledLamps(boolean dev, int serviceIndex) {
		Service getControlledLamps = createService(SERVICE_GET_LAMPS + serviceIndex, dev);
		getControlledLamps.addOutput(OUTPUT_CONTROLLED_LAMPS, Lamp.MY_URI, 0, -1, ppControls);
		return getControlledLamps.getProfile();
	}
	...
	public static ServiceProfile create_getLampInfo(boolean dev) {
		...
		getLampInfo.addFilteringInput(INPUT_LAMP_URI, Lamp.MY_URI, 1, 1, ppControls);
		getLampInfo.addOutput(OUTPUT_LAMP_BRIGHTNESS, TypeMapper.getDatatypeURI(Integer.class), 1, 1, ppBrightness);
		getLampInfo.addOutput(OUTPUT_LAMP_LOCATION, Location.MY_URI, 1, 1, ppLocation);
	...
	public static ServiceProfile create_dim(boolean dev) {
		...
		dim.addInputWithChangeEffect(INPUT_LAMP_BRIGHTNESS, TypeMapper.getDatatypeURI(Integer.class), 1, 1,
				ppBrightness);
		return dim.getProfile();
	}

	public static ServiceProfile create_getIndoors1() {
		Service getIndoors = createService(SERVICE_GET_LAMPS_INDOORS + "_1_", false);
		getIndoors.addInstanceLevelRestriction(
				MergedRestriction.getFixedValueRestriction(PhysicalThing.PROP_PHYSICAL_LOCATION, IndoorPlace.MY_URI),
				ppLocation);
		getIndoors.addOutput(OUTPUT_CONTROLLED_LAMPS, Lamp.MY_URI, 0, 0, ppControls);
	...
	public static ServiceProfile create_getIndoors2() {
		Service getIndoors = createService(SERVICE_GET_LAMPS_INDOORS + "_2_", false);
	...
	public static ServiceProfile create_getIndoors3() {
		Service getIndoors = createService(SERVICE_GET_LAMPS_INDOORS + "_3_", false);
		getIndoors.addInstanceLevelRestriction(
				MergedRestriction.getFixedValueRestriction(PhysicalThing.PROP_PHYSICAL_LOCATION, IndoorPlace.MY_URI),
				ppLocation);
		getIndoors.addFilteringType(INPUT_LAMP_LOCATION, ppLocation);
