...
public class DistributedTest extends ServiceBusTestCase {
	...
	public void testSingleGetLamps() {
		...
		testAllDeployments("SingleProfileGetLamps", ProfileUtil.create_getControlledLamps(true),
				new ArrayListCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp1),
				RequestUtil.getAllLampsRequest(true), checker);
	}

	public void testMultiGetLamps() {
		// scenario: getLamps, 2 callees, allDeployments
		ServiceProfile profile1 = ProfileUtil.create_getControlledLamps(true, 0);
		ServiceProfile profile2 = ProfileUtil.create_getControlledLamps(true, 1);
		CallHandler handler1 = new ArrayListCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp1);
		CallHandler handler2 = new ArrayListCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp2);
		ResponseChecker checker = new ResponseChecker() {
			public void check(ServiceResponse sr) {
				checkResponse2(sr);
			}
		...
		testAllDeployments("TwoProfilesGetLamps", profile1, handler1, profile2, handler2,
				RequestUtil.getAllLampsRequest(true), checker, true);
	}
	...
	public void testMultiGetLampsSameProcess() {
		...
		testAllDeployments("TwoProfilesGetLamps", profile1, handler1, profile2, handler2,
				RequestUtil.getAllLampsRequest(true), checker, false);
	}
	...
	public void testResponseAggCoord() {
		...
		setHandler(COORD, 0, new ObjectCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp1));
		deployProfiles(COORD, 1, ProfileUtil.create_getControlledLamps(true, 1));
		setHandler(COORD, 1, new ArrayListCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp2, lamp3));
	...
	public void testResponseAggListObject1() {
		...
		setHandler(NODE1, 0, new ArrayListCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp1, lamp2));
		deployProfiles(NODE1, 1, ProfileUtil.create_getControlledLamps(true, 1));
		setHandler(NODE1, 1, new ObjectCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp3));
	...
	public void testResponseAggListObject2() {
		...
		setHandler(NODE2, 1, new ObjectCallHandler(ProfileUtil.OUTPUT_CONTROLLED_LAMPS, lamp3));
	...
	public void testResponseAgg2Outputs() {
		...
		setHandler(NODE1, new TwoObjectCallHandler(ProfileUtil.OUTPUT_LAMP_BRIGHTNESS, int11,
				ProfileUtil.OUTPUT_LAMP_LOCATION, room1));
		deployProfiles(NODE2, ProfileUtil.create_getLampInfo(true));
		setHandler(NODE2, new TwoObjectCallHandler(ProfileUtil.OUTPUT_LAMP_BRIGHTNESS, int22,
	...
	public void testSameProcessURI() {
		...
		c.addProfiles(new ServiceProfile[] { ProfileUtil.create_getControlledLamps(true) });
		waitForProfileNumberChange(num);

		// test that adding the same profile will throw
		boolean hasThrown = false;
		try {
			c.addProfiles(new ServiceProfile[] { ProfileUtil.create_getControlledLamps(true) }, true);
		...
		hasThrown = false;
		try {
			c.addProfiles(new ServiceProfile[] { ProfileUtil.create_getControlledLamps(true) }, false);
		} catch (ProfileExistsException e) {
			hasThrown = true;
		}
