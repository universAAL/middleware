...
public class ChannelMessageTest {
    ...
    private PeerCard card = new PeerCard(PeerRole.COORDINATOR, "OSGi",
	    "universAAL");
	...
	assertEquals("Problem serializing/deserializing PeerCard", card,
		parsed.getSender());
	assertEquals("Problem serializing/deserializing Content",
		msg.getContent(), parsed.getContent());
	...
	try {
		...
		ChannelMessage chMsg = parser.fromJson(msg,
			ChannelMessage.class);
		msg = br.readLine();
		// assertEquals("Failed to handle message"+msg,
		// chMsg.toString(), msg);
	    }
	} catch (IOException e) {
	    e.printStackTrace();
