...
public class BusTrackerTestCase extends BusTestCase {
	...
	private static class BusListener implements IBusMemberRegistryListener {
		...
		public synchronized void busMemberAdded(BusMember member, BusType type) {
			...
			System.out.println("       " + member.getURI() + "   " + type.toString());
		...
		public synchronized void regParamsAdded(String busMemberID, Resource[] params) {
		...
		public synchronized void regParamsRemoved(String busMemberID, Resource[] params) {
	...
	private void checkAnswer(Checker checker) {
		...
		while (!checker.check()) {
			...
			if (i > timeout) {
				System.out.println("-------------------------");
				new Exception("Error: a timeout occured").printStackTrace(System.out);
				System.out.println("  current values:");
				System.out.println("     busMemberAddedDone:   " + listener.busMemberAddedDone);
				System.out.println("     busMemberRemovedDone: " + listener.busMemberRemovedDone);
				System.out.println("     regParamsAddedDone:   " + listener.regParamsAddedDone);
				System.out.println("     regParamsRemovedDone: " + listener.regParamsRemovedDone);
	...
	private boolean checkOps(boolean busMemberAddedDone, boolean busMemberRemovedDone, boolean regParamsAddedDone,
	...
	public void testRegParams() {
		...
		final MyServiceCallee c = new MyServiceCallee(new ServiceProfile[] { p1, p2 });
