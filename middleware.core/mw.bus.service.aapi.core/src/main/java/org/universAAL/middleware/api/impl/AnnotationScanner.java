		...
		for (Method m : scannedClazz.getMethods()) {
			...
			for (Annotation a : m.getAnnotations()) {
				if (a.annotationType().equals(ServiceOperation.class)) {
					...
					methodChangeEffectsAnnotations.addAll(Arrays.asList(((ChangeEffects) a).value()));
				}
			}
			if (methodOutputAnnotations.isEmpty()) {
				...
				if (returnType != void.class) {
					...
					if (name.startsWith("get") && name.length() > 3) {
						outputName = String.format("%s%s", Character.toLowerCase(name.charAt(3)), name.substring(4));
					}
			...
			for (int i = 0; i < parameterAnnotations.length; i++) {
				...
				if (input == null) {
					throw new SimplifiedRegistrationException(
							"Does not found @input annotation for " + i + " parameter of " + m.getName() + " method!");
				}
