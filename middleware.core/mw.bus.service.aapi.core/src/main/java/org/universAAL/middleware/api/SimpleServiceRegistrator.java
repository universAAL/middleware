...
public class SimpleServiceRegistrator {
	 ...
	 */
	private String exctractUriFromResourceClass(OntologyClasses resourceClass) throws SimplifiedRegistrationException {
	 ...
	 * @throws SimplifiedRegistrationException
	 */
	public void registerService(Object o) throws SimplifiedRegistrationException {
		...
		for (Method m : annotatedInterface.getMethods()) {
			...
			for (Annotation a : m.getAnnotations()) {
				if (a.annotationType().equals(ServiceOperation.class)) {
					...
					if (annotatedMethods
							.containsKey(AnnotationScanner.createServiceUri(namespace, name, serviceName))) {
						throw new SimplifiedRegistrationException(
								"Duplicate name definition for @ServiceOperation: " + serviceName);
					}
					annotatedMethods.put(AnnotationScanner.createServiceUri(namespace, name, serviceName), m);
				} else if (a.annotationType().equals(Output.class) || a.annotationType().equals(Outputs.class)) {
					Output[] temp = null;
					...
					for (int i = 0; i < temp.length; i++) {
						...
						if ("".equals(assignedName)) {
							outputName = namespace + "output" + (outputs.size() + 1);
						} else {
					...
					methodChangeEffects.addAll(Arrays.asList(((ChangeEffects) a).value()));
				}
			}
			...
			if (outputs.size() == 0) {
				...
				if (returnType != void.class) {
					...
					if (methodName.startsWith("get") && methodName.length() > 3) {
						methodName = String.format("%s%s", Character.toLowerCase(methodName.charAt(3)),
			...
			if (serviceName != null) {
				annotatedOutputNames.put(AnnotationScanner.createServiceUri(namespace, name, serviceName), outputs);
				annotatedOutputs.put(AnnotationScanner.createServiceUri(namespace, name, serviceName),
				...
				if (parameterAnnotations.length != m.getParameterTypes().length) {
					...
					throw new SimplifiedRegistrationException(
							"Invalid number of @Input annotations for @ServiceOperation. Expected:  "
									+ m.getParameterTypes().length + " got " + parameterAnnotations.length);
				}
				...
				for (int i = 0; i < parameterAnnotations.length; i++) {
					...
					if (methodParameterNames.contains(parameterName)) {
						throw new SimplifiedRegistrationException("Duplicate parameter name definition: '"
								+ parameterName + "' for @ServiceOperation: " + serviceName);
					}
					methodParameterNames.add(parameterName);
				}
				annotatedMethodsParametersNames.put(AnnotationScanner.createServiceUri(namespace, name, serviceName),
						methodParameterNames);
				annotatedMethodsParameters.put(AnnotationScanner.createServiceUri(namespace, name, serviceName),
						methodParameterAnnotations);
				profiles.add(createServiceProfileForMethod(namespace, name, serviceName, methodParameterAnnotations,
						methodParameterTypes, methodOutputAnnotations, m.getReturnType(), methodChangeEffects,
		...
		ServiceCalleeWrapper wrapper = new ServiceCalleeWrapper(mc, profiles.toArray(new ServiceProfile[0]), o,
				namespace, name, annotatedMethods, annotatedOutputNames, annotatedOutputs,
	 ...
	 */
	private void registerOntology(String namespace, String name, String ontologyUri)
			throws SimplifiedRegistrationException {
		OntologyManagement.getInstance().register(mc, new SimpleOntology(namespace + name, ontologyUri));
	}
	 ...
	 * Service) and created Inputs, Outputs, and ChangeEffects. - for
	 ...
	 * @throws SimplifiedRegistrationException
	 */
	private ServiceProfile createServiceProfileForMethod(String namespace, String name, String serviceName,
			List<Input> methodParametersAnnotations, Class<?>[] methodParameterTypes,
			List<Output> methodOutputAnnotations, Class<?> methodOutputType, List<ChangeEffect> methodChangeEffects,
			String ontologyUri, Cardinality defaultOutputCardinality) throws SimplifiedRegistrationException {
		SimplifiedApiService srv = SimplifiedApiService.createService(namespace, serviceName, ontologyUri);
		...
		for (int i = 0; i < methodParametersAnnotations.size(); i++) {
			...
			if (input.propertyPaths().length > 0) {
				...
				srv.createInputWrapper(parameterURI, filteringClass, defaultInputCardinality, input.propertyPaths());
			}
		}
		...
		for (int i = 0; i < methodOutputAnnotations.size(); i++) {
			...
			if (output.propertyPaths().length > 0) {
				...
				filteringClass = methodOutputAnnotations.get(i).filteringClass();
				...
				srv.addOutputWrapper(parameterURI, filteringClass, card, propertyPaths);
			}
		}
		for (int i = 0; i < methodChangeEffects.size(); i++) {
			ChangeEffect ce = methodChangeEffects.get(i);
			srv.addChangeEffectWrapper(ce.propertyPaths(), ce.value(), ce.valueType());
		}
