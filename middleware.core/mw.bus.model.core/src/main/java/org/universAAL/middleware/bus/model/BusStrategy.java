...
public abstract class BusStrategy extends Thread {
		...
		synchronized (queue) {
			if (queue.isEmpty()) {
				...
				for (int i = 0; !inserted && i < queue.size(); i++) {
					...
					if (m.getSender().getPeerID().equals(current.getSender().getPeerID())
	...
	protected ChannelMessage buildChannelMessage(BusMessage m) {
		...
		ChannelMessage channelMessage = new ChannelMessage(bus.getPeerCard(), m.toString(), channelName);
	...
	protected void send(BusMessage message) {
		...
		while (!commModule.hasChannel(busModule.getID())) {
			...
			if (cnt % 50 == 0) // log a message every 5 seconds
				LogUtils.logError(busModule, BusStrategy.class, "send",
						new Object[] { "The communication connector is not yet configured correctly (channel ",
								busModule.getID(),
		...
		if (isBroadcast(receivers)) {
			...
			commModule.sendAll(buildChannelMessage(message), Arrays.asList(receivers), bus);
		}
	}
