...
public abstract class AbstractBus implements Broker, MessageListener {
	...
	public static void initBrokerage(ModuleContext mc, SpaceManager aalSpaceMgr, CommunicationModule commModule) {
		if (myContext != null) {
			...
			// "initBrokerage",
			// new Object[] {
			// "The init method was called already, it cannot be called a second
			// time. The original caller was ",
			// myContext.getID(), " and the current caller is ",
			// mc.getID() }, null);
			// return;
			LogUtils.logDebug(myContext, AbstractBus.class, "initBrokerage",
					new Object[] { "The init method is called again. The original caller was ", myContext.getID(),
	...
	protected AbstractBus(ModuleContext module, String brokerName) {
		...
		if (communicationModule != null)
			...
			LogUtils.logDebug(myContext, AbstractBus.class, "AbstractBus",
	...
	public final String register(ModuleContext module, BusMember m, BusMemberType type) {
		...
		synchronized (registry) {
			...
			LogUtils.logDebug(context, AbstractBus.class, "register",
					new Object[] { "New bus member has registered: ", id }, null);
			return id;
		}
	}
	...
	public void messageReceived(ChannelMessage message) {
		if (message != null) {
			BusMessage busMessage = (BusMessage) unmarshall(message.getContent());
			LogUtils.logDebug(context, AbstractBus.class, "messageReceived", new Object[] { context.getID(),
					" - Received message from peer ", message.getSender().getPeerID(), ":\n", message.toString() },
					null);
			...
			LogUtils.logWarn(context, AbstractBus.class, "messageReceived",
					new Object[] { context.getID(), " - message is null; ignoring.", }, null);
		}
	}
