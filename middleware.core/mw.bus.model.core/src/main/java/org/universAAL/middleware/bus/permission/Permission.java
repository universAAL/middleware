...
public class Permission {
	...
	public static void init(ModuleContext mc) {
		if (Permission.mc != null)
			throw new SecurityException("Permission can only be initialized once");

		Permission.mc = mc;
	}

	public static Permission[] fromManifest(ModuleContext mc, String brokerName, boolean isAdvertisement, boolean log) {
		return fromManifest(mc, brokerName, isAdvertisement ? ADVERTISEMENT : REQUIREMENT, log);
	}

	public static Permission[] fromManifest(ModuleContext mc, String brokerName, String type, boolean log) {
		if (brokerName == null)
		...
		if (brokerName.endsWith("_osgi"))
			brokerName = brokerName.substring(0, brokerName.length() - "_osgi".length());
		type = type.replace(".", "_");
	...
	public static Permission[] parsePermission(String entry) {
		...
		for (int i = 1; i < titlesplit.length; i++) {
			...
			if (restsplit.length != 2) {
				LogUtils.logDebug(mc, Permission.class, "parsePermission", new Object[] {
						"Manifest corrupt: each manifest entry must" + " be separated by the three defined values"
								+ " for title, description and serialization." + " This manifest entry (title: "
								+ p.title + ") has ",
						restsplit.length < 2 ? "no" : "more than one", " description separator" }, null);
				return new Permission[0];
			}
			...
			if (((i != titlesplit.length - 1) && (restsplit.length != 2))
				...
				LogUtils.logDebug(mc, Permission.class, "parsePermission",
						new Object[] { "Manifest corrupt: each manifest entry must"
								...
								+ " for title, description and serialization." + " This manifest entry (title: "
								+ p.title + ") does not have a serialization separator or"
			...
			if (o == null) {
				LogUtils.logDebug(mc, Permission.class, "parsePermission",
						new Object[] { "Manifest corrupt: the serialization of" + " the manifest entry (title: "
								+ p.title + ") could not be deserialized." },
						null);
				continue;
			}
			try {
				...
				LogUtils.logDebug(mc, Permission.class, "parsePermission",
						new Object[] { "Manifest corrupt: the serialization of" + " the manifest entry (title: "
								+ p.title + ") could not be deserialized as a valid matchable resource." },
						null);
				continue;
			}
			perms.add(p);
