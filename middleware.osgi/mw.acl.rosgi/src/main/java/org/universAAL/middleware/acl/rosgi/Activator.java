 ...
 * 
 */
public class Activator implements P2PConnector, BundleActivator,
	...
	context.registerService(new String[] { P2PConnector.class.getName(),
		ServiceDiscoveryListener.class.getName() }, this, null);
    }
	...
	System.out.println("acl_rOSGi discovered a peer with class name "
		+ serviceInterface);
	...
	synchronized (remote) {
	    try {
		remote.connect(uri);
		Object proxy = remote.getRemoteService(remote
			.getRemoteServiceReference(uri));
		System.out.println("The acl_rOSGi proxy is of type "
			+ proxy.getClass().getName());
		if (!(proxy instanceof SodaPopPeer)
			|| (localInstance != null && localInstance.getID()
				.equals(((SodaPopPeer) proxy).getID())))
		    return;
	...
	System.out.println("acl_rOSGi lost a peer with class name "
		+ serviceInterface);
	if (!serviceInterface.equals(SodaPopPeer.class.getName()))
	    return;
	...
	synchronized (remote) {
	    ...
	    if (id != null)
		for (Iterator i = listeners.iterator(); i.hasNext();)
		    ((PeerDiscoveryListener) i.next()).noticeLostPeer(id,
			    getProtocol());
	}
	...
	synchronized (remote) {
		...
		try {
		    listener
			    .noticeNewPeer((SodaPopPeer) remote
				    .getRemoteService(remote
					    .getRemoteServiceReference((URI) i
						    .next())), getProtocol());
		} catch (Exception e) {
		}
	    }
	...
	synchronized (remote) {
	    ServiceRegistration sreg = (ServiceRegistration) bridgedRegs
		    .remove(peerID);
	    if (sreg != null)
		sreg.unregister();
	}
	...
	synchronized (remote) {
	    ...
	    bridgedRegs.put(newPeer.getID(), context.registerService(
		    SodaPopPeer.class.getName(), newPeer, properties));
	}
    }
	...
	synchronized (remote) {
	    final Hashtable properties = new Hashtable();
	    properties.put(RemoteOSGiService.R_OSGi_REGISTRATION, Boolean.TRUE);
	    context.registerService(SodaPopPeer.class.getName(), localInstance,
		    properties);
	}
    }
}
