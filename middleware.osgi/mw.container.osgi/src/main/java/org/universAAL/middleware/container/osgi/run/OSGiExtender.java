...
public class OSGiExtender implements SynchronousBundleListener {
	...
	private String getClassNameFromEntry(String entryStr) {
		...
		if (entryParsed.length == 2) {
			...
			if (index >= 0) {
				...
				realEntryPath = realEntryPath.substring(0, realEntryPath.length() - 6);
	...
	private Class loadActivatorClass(Bundle bundle, String className) throws Exception {
		if (className != null) {
	...
	private String buildErrorMsg(ArrayList activators, Bundle bundle) throws Exception {
	...
	private void doStart(final Bundle bundle) throws Exception {
		try {
			Enumeration entries = bundle.findEntries("", "*Activator*.class", true);
			if (entries != null) {
				...
				while (entries.hasMoreElements()) {
					...
					try {
						activatorClass = loadActivatorClass(bundle, getClassNameFromEntry(entryUrl.toString()));
					} catch (Throwable e) {
						LogUtils.logError(Activator.mc, getClass(), "doStart",
								new String[] { "Unable to load Activator class." }, e);
					}
				...
				if (!activatorsClasses.isEmpty()) {
					...
					if (activatorsClasses.size() == 1) {
						...
						ModuleActivator theActivator = (ModuleActivator) c.newInstance();
						executor = new ActivatorExecutor(theActivator, bundle);
					} else {
						String errorMsg = buildErrorMsg(activatorsClasses, bundle);
						executor = new ActivatorExecutor(errorMsg, bundle);
					}
					executor.start();
					bundleIdToActivatorExecutor.put(new Long(bundle.getBundleId()), executor);
				}
			}
		} catch (Exception t) {
	...
	private void doStop(Bundle bundle) throws Exception {
		if (bundle.getBundleId() != 0) {
			...
			if (executor != null) {
				executor.stop();
				bundleIdToActivatorExecutor.remove(new Long(bundle.getBundleId()));
			}
		}
	}
	...
	private class ActivatorExecutor {
		...
		public ActivatorExecutor(String errorMsg, Bundle b) {
			...
			this.b = b;
			this.logger = LoggerFactory.getLogger("org.universAAL." + b.getSymbolicName());
		}

		private boolean logError() {
			if (errorMsg != null) {
				logger.error(errorMsg, new IllegalStateException(
						"Bundle " + b.getSymbolicName() + " has more than one ModuleActivator"));
				return true;
			}
			return false;
		}
