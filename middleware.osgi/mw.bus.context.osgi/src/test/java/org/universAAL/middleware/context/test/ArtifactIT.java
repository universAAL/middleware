...
public class ArtifactIT extends IntegrationTest {
	...
	protected void logInfo(String format, Object args) {
		...
		LogUtils.logInfo(Activator.getModuleContext(), getClass(), callingMethod.getMethodName(),
	...
	protected void logError(Throwable t, String format, Object args) {
		StackTraceElement callingMethod = Thread.currentThread().getStackTrace()[2];
		LogUtils.logError(Activator.getModuleContext(), getClass(), callingMethod.getMethodName(),
	...
	public void testDynamicImport() {
		ContextEvent cev1 = ContextEvent.constructSimpleEvent(DUMMYUSER, USER, HAS_LOCATION, LevelRating.high);
	}
	...
	public void testCreateContextPublisher() {
		...
		logInfo("Created Default Context Publisher with full Provider Info", null);

		// Incorrectly create a CP without info
		try {
			pub5 = new DummyContextPublisher(null);
			// Assert.notNull(null,"Allowed creation of a Context Publisher with
			// null provider info");
		} catch (Exception e) {
			Assert.notNull(e);
			logInfo("Properly launched exception creating bad publisher %s", e.toString());
		}
		...
		try {
			info = new ContextProvider();
			pub4 = new DefaultContextPublisher(Activator.getModuleContext(), info);
			// Assert.notNull(null,"Allowed creation of a Context Publisher with
			// null provider info");
		} catch (Exception e) {
			Assert.notNull(e);
			logInfo("Created Default Context Publisher without full Provider Info", null);
		}
	...
	public void testCreateContextSubscriber() {
		...
		try {
			...
			Assert.notNull(e);
			logInfo("Properly launched exception creating bad subscriber %s", e.toString());
		}
	...
	public void testSendContextEvent() {
		...
		ContextEvent cev3 = new ContextEvent(info2, ContextProvider.PROP_CONTEXT_PROVIDER_TYPE);
		pub.publish(cev3);
		...
		try {
			...
			Assert.notNull(e);
			logInfo("Properly launched exception sending null event %s", e.toString());
		}
	}
	...
	public void testReceiveContextEvent() {
		...
		cepA.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_SUBJECT,
				new Resource(DUMMYUSER + "right")));
		cepA.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE,
				new Resource(HAS_LOCATION)));

		cepA.addRestriction(MergedRestriction
		...
		cep2.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_SUBJECT,
				new Resource(DUMMYUSER + "right")));
		cep2.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE,
				new Resource(HAS_LOCATION)));
		...
		ContextEvent cev1 = ContextEvent.constructSimpleEvent(DUMMYUSER + "right", USER, HAS_LOCATION, LOCATION);
		pub.publish(cev1);
		try {
		...
		retrieved = null;
		ContextEvent cev2 = ContextEvent.constructSimpleEvent(DUMMYUSER + "wrong", USER, HAS_LOCATION, LOCATION);
		pub.publish(cev2);
		try {
			retrieved = (Integer) queue.poll(timeout, TimeUnit.MILLISECONDS);
	...
	public void testSubscriptions() {
		...
		cep1.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_SUBJECT, User.MY_URI));
		c1 = new SyncContextSubscriber(new ContextEventPattern[] { cep1 }, 1);

		ContextEventPattern cep2 = new ContextEventPattern();
		cep2.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_SUBJECT, User.MY_URI));
		cep2.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE,
		...
		cep3.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_SUBJECT, User.MY_URI));
		cep3.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE,
				User.PROP_PHYSICAL_LOCATION));
		cep3.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_OBJECT, Location.MY_URI));
		c3 = new SyncContextSubscriber(new ContextEventPattern[] { cep3 }, 3);

		ContextEventPattern cep4 = new ContextEventPattern();
		cep4.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE,
				User.PROP_PHYSICAL_LOCATION));
		cep4.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_OBJECT, Location.MY_URI));
		c4 = new SyncContextSubscriber(new ContextEventPattern[] { cep4 }, 4);

		ContextEventPattern cep5 = new ContextEventPattern();
		cep5.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_SUBJECT, User.MY_URI));
		cep5.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_OBJECT, Location.MY_URI));
		c5 = new SyncContextSubscriber(new ContextEventPattern[] { cep5 }, 5);

		ContextEventPattern cep6 = new ContextEventPattern();
		cep6.addRestriction(MergedRestriction.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE,
				User.PROP_PHYSICAL_LOCATION));
		c6 = new SyncContextSubscriber(new ContextEventPattern[] { cep6 }, 6);

		ContextEventPattern cep7 = new ContextEventPattern();
		cep7.addRestriction(MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_OBJECT, Location.MY_URI));
		...
		cpublisher = new DefaultContextPublisher(Activator.getModuleContext(), p);
		...
		try {
			...
			Assert.isTrue(!good.isEmpty(), "Not received an expected subscribed event. Received subscriptions " + good);
	...
	protected class DummyContextSubscriber extends ContextSubscriber {
		...
		protected DummyContextSubscriber(ContextEventPattern[] initialSubscriptions) {
	...
	protected class SyncContextSubscriber extends ContextSubscriber {
		...
		protected SyncContextSubscriber(ContextEventPattern[] initialSubscriptions) {
			super(Activator.getModuleContext(), initialSubscriptions);
		}

		protected SyncContextSubscriber(ContextEventPattern[] initialSubscriptions, int index) {
			super(Activator.getModuleContext(), initialSubscriptions);
			ind = index;
		}
		...
		public void handleContextEvent(ContextEvent event) {
			try {
				boolean sent = queue.offer(new Integer(ind), timeout, TimeUnit.MILLISECONDS);
				if (!sent) {
