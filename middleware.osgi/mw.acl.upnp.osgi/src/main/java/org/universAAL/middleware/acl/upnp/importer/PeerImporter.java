...
public class PeerImporter implements ServiceListener {
    ...
    /**
     * First an LDAP filter is configured in order to fetch already existent
     * UPnPDevice from the registry. For every discovered service the private
     * method addRemotePeer is called.
     * 
     * @param context
     * @param listener
     */
	...
	String filter = "(&" + "(" + Constants.OBJECTCLASS + "="
		...
		+ UPnPDevice.UPNP_EXPORT + "=*) )" + "(" + UPnPDevice.TYPE
		+ "=" + SodaPopDevice.TYPE + ")" + ")";
	...
	synchronized (LOCK) {
	    ...
	    try {
		services = context.getServiceReferences(UPnPDevice.class
			.getName(), filter);
		if (services != null) {
    ...
    /**
     * This method implements the PeerImporter business logic: -creates a
     * SodaPopProxy for every discovered UPnPDevice -notify every listener about
     * the existence of a new SodaPopPeer by passing the brand new Proxy.
     * 
	...
	for (Iterator i = listeners.iterator(); i.hasNext();) {
	    ...
	    ((PeerDiscoveryListener) i.next()).noticeNewPeer(proxy,
     ...
     * -fetch the previously created SodaPopProxy -notify all the listener by
     * passing the realted Proxy
     * 
     * @param serviceReference
     */
	...
	synchronized (LOCK) {
		...
		listener.noticeNewPeer((SodaPopPeer) proxies.get(i.next()),
			UPnPDevice.DEVICE_CATEGORY);
	    }
	    listeners.add(listener);
	}
