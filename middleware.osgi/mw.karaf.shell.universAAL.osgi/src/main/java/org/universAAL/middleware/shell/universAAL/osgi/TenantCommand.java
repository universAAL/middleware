...
public class TenantCommand extends OsgiCommandSupport {
	...
	protected Object doExecute() throws Exception {
		...
		ServiceReference ref = bundleContext.getServiceReference(TenantManager.class.getName());
		if (ref != null) {
			tenantManager = (TenantManager) bundleContext.getService(ref);
			System.out.println("--------Tenants connected to this instance----------");
			for (String tenantID : tenantManager.getTenants().keySet()) {
				System.out.println(
						"Tenant ID: " + tenantID + " - Tenant Description:" + tenantManager.getTenants().get(tenantID));
			}
		}
		return null;
	}
