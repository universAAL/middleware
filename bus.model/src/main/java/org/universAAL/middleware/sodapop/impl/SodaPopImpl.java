...
//		logger.info(CryptUtil.init(Activator.CONF_DIR));
		...
		synchronized (this.context) {
			...
			try {
				ServiceReference[] connectors = context.getAllServiceReferences(
				...
				if (connectors != null)
					...
					for (int i=0; i<connectors.length; i++) {
						P2PConnector c = (P2PConnector) context.getService(connectors[i]);
						c.register(this);
...
//		startTime = System.currentTimeMillis();
	...
	public void serviceChanged(ServiceEvent se) {
		synchronized (this.context) {
			...
			if (service instanceof P2PConnector  &&  se.getType() == ServiceEvent.REGISTERED) {
	...
	private void forwardBusesToPeer(Dispatcher dispatcher) {
		...
		if (myBusses.length()>0) myBusses.deleteCharAt(myBusses.length()-1);
		PeerCommand message = new PeerCommand(PeerCommand.NOTICE_PEER_BUSES, myID, myBusses.toString());
		dispatcher.queue.enqueue(message);
		logger.info("call remote NOTICE_PEER_BUSES on '{}'", dispatcher.peer.getID());
	...
	public void join(AbstractBus b) {
		...
		synchronized (localBusses) {
			if (localBusses.containsKey(busName))
				throw new RuntimeException("A bus with the name '" 
						+ busName
			...
			while(list.hasMoreElements() ){
					...
					synchronized (contactedPeers) {
						if (contactedPeers.contains(dispatcher))	{
							PeerCommand command = new PeerCommand(PeerCommand.JOIN_BUS,myID,busName); 
							dispatcher.queue.enqueue(command);
							logger.info("call remote JOIN_BUS on '{}'", dispatcher.peer.getID());
						}
					}
	...
	public void leave(AbstractBus b) {
		...
		synchronized (remoteSodapops) {
			...
			while(list.hasMoreElements() ){
				...
				synchronized (contactedPeers) {
					if (contactedPeers.contains(dispatcher))	{
						PeerCommand command = new PeerCommand(PeerCommand.LEAVE_BUS,myID,busName); 
						dispatcher.queue.enqueue(command);
						logger.info("call remote LEAVE_BUS on '{}'", dispatcher.peer.getID());
					}
				}
			}
		}
	...
	public int propagateMessage(AbstractBus b, Message m) {
		...
		synchronized (peersOnBus) {
			...
			if (peersOnThisBus != null
					&&  !peersOnThisBus.isEmpty()
					&&  b == localBusses.get(busName)) 
			{
				/*try {
					cipher = CryptUtil.encrypt(msg);
				} catch (Exception e) {
					logger.warn("Message encryption failed - trying to send it as clear text!", e);
				}*/
				PeerCommand command  = new PeerCommand(PeerCommand.PROCESS_MESSAGE,busName,cipher);
				...
				if (receivers == null  ||  receivers.length == 0){
					...
					synchronized (remoteSodapops) {
						while (peerList.hasNext() ) {
							...
							((Dispatcher) dispatchers.get(peerID)).queue.enqueue(command);
							result++;
						}
					}
				}
					...
					synchronized (remoteSodapops) {
						for (int i=0; i<receivers.length; i++)
							if (peersOnThisBus.contains(receivers[i])) {
								((Dispatcher) dispatchers.get(receivers[i])).queue.enqueue(command);
								result++;
							}
					}
				}
	...
	private void replyBusesToPeer(Dispatcher dispatcher) {
		...
		if (myBusses.length()>0){
			...
			PeerCommand message = new PeerCommand(PeerCommand.REPLY_PEER_BUSES, myID, myBusses.toString());
			dispatcher.queue.enqueue(message);
			logger.info("call remote REPLY_PEER_BUSES on '{}'", dispatcher.peer.getID());
		}
		synchronized (contactedPeers) {
			contactedPeers.add(dispatcher);			
		}
		...
		synchronized (localBusses) {
			...
			if (b != null) {
				try {
					...
					logger.warn(
							"Message processing aborted due to the following exception:",e);
				}
			} else
				logger.warn(
						"Bus '" + busName + "' is absent locally!");
		}
		if (m == null)
			logger.debug(
					"Message '\n" + msg
			...
			logger.debug(
					"Message '" + m.getSourceTimeOrder()
					+ "' received on bus '" + busName
					+ "' from peer '" + m.getSource() + "'!");
	}
...
//	synchronized private final void logger(int level, String msg,Exception ex){
////		logBuffer.append(msg).append("\n)");
//		stopTime = System.currentTimeMillis();
//		if (log != null)
