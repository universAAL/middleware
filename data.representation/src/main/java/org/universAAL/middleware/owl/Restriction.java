...
public class Restriction extends ClassExpression {
    ...
    public static final Restriction getAllValuesRestriction(String propURI,
	...
	return getAllValuesRestrictionWithCardinality(propURI, TypeURI
		.asTypeURI(typeURI), max, min);
    }

    public static final Restriction getCardinalityRestriction(String propURI,
	    int max, int min) {
    ...
    public static final Restriction getFixedValueRestriction(String propURI,
	    Object o) {
	...
	ClassExpression all = (ClassExpression) tmp.props
	...
	ClassExpression all = (ClassExpression) props
		.get(PROP_OWL_ALL_VALUES_FROM);
	...
	if (all instanceof Intersection)
		...
		if (tmp instanceof Restriction
			&& ((Restriction) tmp).getOnProperty().equals(
				pathElement))
	    ...
	    return ManagedIndividual.getClassRestrictionsOnProperty(all
		    .getURI(), pathElement);
	return (all instanceof Restriction && ((Restriction) all)
		.getOnProperty().equals(pathElement)) ? (Restriction) all
		: null;
    }
	...
	if (all instanceof Intersection) {
		...
		ClassExpression tmp = (ClassExpression) i.next();
		if (tmp instanceof Restriction
			&& ((Restriction) tmp).getOnProperty().equals(propURI))
		    return (Restriction) tmp;
	    }
	} else if (all instanceof Restriction
		&& ((Restriction) all).getOnProperty().equals(propURI)) {
	...
	if (o != null) {
		...
		if (cloned == null || cloned.size() == context.size())
		    ...
		    // TODO: because the compatibility was conditional, there is
		    // still a chance to
		    // return true by adopting complement conditions into
		    // context
		    return false;
	    else
		return true;
	...
	ClassExpression myValues = (ClassExpression) props
		.get(PROP_OWL_SOME_VALUES_FROM);
	if (myValues != null
		&& !myValues.isDisjointWith((ClassExpression) r.props
			.get(PROP_OWL_SOME_VALUES_FROM), cloned))
	    return false;

	myValues = (ClassExpression) props.get(PROP_OWL_ALL_VALUES_FROM);
	if (myValues != null
		&& myValues.isDisjointWith((ClassExpression) r.props
			.get(PROP_OWL_ALL_VALUES_FROM), cloned)) {
	    synchronize(context, cloned);
	    return true;
	}
	...
	return getOnProperty() != null
		&& (props.containsKey(PROP_OWL_ALL_VALUES_FROM)
			...
			|| props.containsKey(PROP_OWL_MIN_CARDINALITY) || props
			.containsKey(PROP_OWL_SOME_VALUES_FROM));
    }
	...
	if (o != null
		&& !props.containsKey(PROP_OWL_HAS_VALUE)
		&& !(props.containsKey(PROP_OWL_ALL_VALUES_FROM) && other.props
			.containsKey(PROP_OWL_ALL_VALUES_FROM)))
	...
	ClassExpression myValues = (ClassExpression) props
		.get(PROP_OWL_SOME_VALUES_FROM);
	if (myValues != null
		&& !myValues.matches((ClassExpression) other.props
			.get(PROP_OWL_SOME_VALUES_FROM), cloned))
	    return false;
    ...
    public Restriction mergeWithNewCardinality(Restriction other, int max,
	    int min) {
	Object o = props.get(PROP_OWL_ON_PROPERTY);
	...
	if (PROP_OWL_ON_PROPERTY.equals(propURI)) {
		...
		props.put(PROP_OWL_ON_PROPERTY, new Resource(((Resource) o)
			.getURI()));
	    return;
	}
	...
	ClassExpression all = (ClassExpression) props
		.get(PROP_OWL_ALL_VALUES_FROM);
	ClassExpression some = (ClassExpression) props
		.get(PROP_OWL_SOME_VALUES_FROM);
	...
	if (PROP_OWL_ALL_VALUES_FROM.equals(propURI)) {
	    ...
	    if (o instanceof ClassExpression
		    && all == null
		    && (some == null || (max != 1 && ((ClassExpression) o)
			    .matches(some, null))))
	    ...
	    if (o instanceof Integer
		    ...
		    && (((Integer) o).intValue() > 1
			    || (((Integer) o).intValue() == 1 && (some == null || all == null)) || (((Integer) o)
			    .intValue() == 0
			    && all == null && some == null)))
	    ...
	    if (o instanceof ClassExpression
		    && some == null
		    && (all == null || ((ClassExpression) all).matches(
			    (ClassExpression) o, null))) {
		if (all != null && max == 1)
