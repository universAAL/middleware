...
public abstract class Service extends ManagedIndividual {
    ...
    public static final String OWLS_SERVICE_NAMESPACE = OWLS_NAMESPACE_PREFIX
	    + "Service.owl#";
    ...
    protected static final boolean addRestriction(MergedRestriction r,
	    String[] toPath, Hashtable restrictions) {
	if (toPath == null || toPath.length == 0 || restrictions == null
		|| r == null)
	    return false;
	MergedRestriction root = (MergedRestriction) restrictions
		.get(toPath[0]);
     ...
     */
    protected static final MergedRestriction getRestrictionOnPropPath(
	    Hashtable restrictions, String[] propPath) {
	...
	if (addRestriction(r, toPath, instanceLevelRestrictions)) {
		...
		props.put(PROP_NUMBER_OF_VALUE_RESTRICTIONS, new Integer(
			++numberOfValueRestrictions));
	    props.put(PROP_INSTANCE_LEVEL_RESTRICTIONS,
		    getRestrictions(instanceLevelRestrictions));
	    return true;
	}
	return false;
    }
	...
	if (propURI == null)
	    return null;
	MergedRestriction r = (MergedRestriction) instanceLevelRestrictions
		.get(propURI);
