 ...
 * a gauge, or a reasoner. A conroller is a context provider that has the control over some
 * context element and hence can report about changes in its/their state(s) or answer
 * queries about that. A gauge is a sensor wrapper; sensors measure real things, such as
 * weight, temperature, the angle to a satellite, etc. A reasoner uses different sources
 * (including other context providers) and infers the state of a context element using
 * different methods, such as aggregation, statistical analysis, and logical deduction, to
 * name a few.   
 * 
 * @author mtazari
 *
...
public class ContextProviderType extends ManagedIndividual {
	...
	static {
		MY_URI = ContextEvent.PERSONA_CONTEXT_NAMESPACE + "ContextProviderType";
	...
	public static ManagedIndividual getIndividualByURI(String instanceURI) {
		return (instanceURI != null
				&&  instanceURI.startsWith(ContextEvent.PERSONA_CONTEXT_NAMESPACE))?
						valueOf(instanceURI.substring(ContextEvent.PERSONA_CONTEXT_NAMESPACE.length()))
						: null;
	}
 ...
 // prevent the usage of the default constructor
    ...
    private ContextProviderType(int order) {
    	super(ContextEvent.PERSONA_CONTEXT_NAMESPACE + names[order]);
        this.order = order;
    }
