...
/**
 * A special case of {@link Label labels} accepting other choice {@link ChoiceItem items} and lists
 * as its children and hence serving as an inner node in a hierarchy of possible choices.
 * Selecting the label of a choice list must make its children accessible.
 * 
 * @author mtazari
 *
 */
public class ChoiceList extends Label {
	public static final String MY_URI = Form.PERSONA_DIALOG_NAMESPACE + "ChoiceList";

	/**
	 * Property for accessing the list of items and sublists contained in a choice list.
	 */
	public static final String PROP_CHILDREN = Form.PERSONA_DIALOG_NAMESPACE + "subchoices";
	 ...
	 * 
	 * @param item The item to be added to this choice list.
	 */
	 ...
	 * 
	 * @param sublist The sublist to be added to this choice list.
	 */
	...
	ChoiceItem findItem(String label) {
		...
		if (children != null)
			for (Iterator i = children.iterator(); result == null  &&  i.hasNext();) {
				Object child = i.next();
				if (child instanceof ChoiceList)
					result = ((ChoiceList) child).findItem(label);
				else if (child instanceof ChoiceItem  &&  child.toString().equals(label))
